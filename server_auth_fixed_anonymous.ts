// apps/web/src/server.ts

import 'dotenv/config';

import cors from 'cors';

import express, {
  type NextFunction,
  type Request,
  type Response,
} from 'express';

import helmet from 'helmet';

import {
  PrismaClient,
} from '@prisma/client';

import {
  readFile,
  readdir,
  stat,
} from 'node:fs/promises';

import path from 'node:path';

import {
  registerAdminRoutes,
} from './admin.js';

import {
  registerAuthRoutes,
  requireAdminAuth,
} from './services/auth.js';

/* =========================================================
   APP
========================================================= */

const app =
  express();

const db =
  new PrismaClient();

/* =========================================================
   MIDDLEWARE
========================================================= */

app.use(
  helmet({
    contentSecurityPolicy:
      false,
  }),
);

app.use(
  cors(),
);

app.use(
  express.json({
    limit:
      '2mb',
  }),
);

/* =========================================================
   CONFIG
========================================================= */

const PORT =
  Number(
    process.env.WEB_PORT ??
      3010,
  );

const guildId =
  process.env.DISCORD_GUILD_ID;

if (
  !guildId
) {
  console.warn(
    '⚠️ DISCORD_GUILD_ID não foi definido no arquivo .env.',
  );
}

/* =========================================================
   AUTENTICAÇÃO
========================================================= */

registerAuthRoutes(
  app,
);

/* =========================================================
   PROTEÇÃO ADMINISTRATIVA
========================================================= */

app.use(
  [
    '/admin',
    '/api/admin',
    '/api/finance',
    '/api/history',
    '/api/transcripts',
    '/historico',
    '/transcripts',
  ],
  requireAdminAuth,
);

/* =========================================================
   ADMIN PANEL
========================================================= */

if (
guildId
) {
  registerAdminRoutes(
    app,
    db,
    guildId,
  );
}

/* =========================================================
   HELPERS
========================================================= */

function getGuildId(): string {
  if (
    !guildId
  ) {
    throw new Error(
      'DISCORD_GUILD_ID não configurado no .env.',
    );
  }

  return guildId;
}

function getParamString(
  value: unknown,
): string {
  if (
    typeof value === 'string'
  ) {
    return value;
  }

  if (
    Array.isArray(value)
  ) {
    const first = value[0];

    return typeof first === 'string'
      ? first
      : '';
  }

  return '';
}

function escapeHtml(
  value: string,
): string {
  return value
    .replaceAll(
      '&',
      '&amp;',
    )
    .replaceAll(
      '<',
      '&lt;',
    )
    .replaceAll(
      '>',
      '&gt;',
    )
    .replaceAll(
      '"',
      '&quot;',
    )
    .replaceAll(
      "'",
      '&#039;',
    );
}

function formatMoney(
  value: number,
): string {
  return new Intl.NumberFormat(
    'pt-BR',
    {
      style:
        'currency',

      currency:
        'BRL',

      maximumFractionDigits:
        0,
    },
  ).format(
    Number(
      value ??
        0,
    ),
  );
}

function formatDuration(
  seconds: number,
): string {
  const safe =
    Math.max(
      0,
      Math.floor(
        Number(
          seconds ??
            0,
        ),
      ),
    );

  const hours =
    Math.floor(
      safe /
        3600,
    );

  const minutes =
    Math.floor(
      (
        safe %
        3600
      ) /
        60,
    );

  const secs =
    safe %
    60;

  if (
    hours > 0
  ) {
    return (
      `${hours}h ` +
      `${minutes}min`
    );
  }

  if (
    minutes > 0
  ) {
    return (
      `${minutes}min ` +
      `${secs}s`
    );
  }

  return (
    `${secs}s`
  );
}

function formatClock(
  seconds: number,
): string {
  const safe =
    Math.max(
      0,
      Math.floor(
        Number(
          seconds ??
            0,
        ),
      ),
    );

  const hours =
    Math.floor(
      safe /
        3600,
    )
      .toString()
      .padStart(
        2,
        '0',
      );

  const minutes =
    Math.floor(
      (
        safe %
        3600
      ) /
        60,
    )
      .toString()
      .padStart(
        2,
        '0',
      );

  const secs =
    (
      safe %
      60
    )
      .toString()
      .padStart(
        2,
        '0',
      );

  return (
    `${hours}:${minutes}:${secs}`
  );
}

/* =========================================================
   VOICE
========================================================= */

type VoiceRankingEntry = {
  memberId: string;
  name: string;
  seconds: number;
  active: boolean;
  channelId: string | null;
  startedAt: Date | null;
};

function liveSeconds(
  startedAt: Date,
): number {
  return Math.max(
    0,
    Math.floor(
      (
        Date.now() -
        startedAt.getTime()
      ) /
        1000,
    ),
  );
}

async function getTopVoice(
  serverGuildId: string,
  limit = 25,
): Promise<
  VoiceRankingEntry[]
> {

  const finished =
    await db.voiceSession.findMany({
      where: {
        guildId:
          serverGuildId,

        endedAt: {
          not:
            null,
        },
      },

      select: {
        memberId:
          true,

        seconds:
          true,
      },
    });

  const active =
    await db.voiceSession.findMany({
      where: {
        guildId:
          serverGuildId,

        endedAt:
          null,
      },

      select: {
        memberId:
          true,

        channelId:
          true,

        startedAt:
          true,
      },
    });

  const totals =
    new Map<
      string,
      {
        seconds: number;
        active: boolean;
        channelId:
          string | null;
        startedAt:
          Date | null;
      }
    >();

  /*
   * Sessões finalizadas.
   */

  for (
    const session of finished
  ) {

    const seconds =
      Math.max(
        0,
        Number(
          session.seconds ??
            0,
        ),
      );

    const current =
      totals.get(
        session.memberId,
      );

    if (
      current
    ) {

      current.seconds +=
        seconds;

      continue;
    }

    totals.set(
      session.memberId,
      {
        seconds,

        active:
          false,

        channelId:
          null,

        startedAt:
          null,
      },
    );
  }

  /*
   * Sessões ativas.
   */

  for (
    const session of active
  ) {

    const seconds =
      liveSeconds(
        session.startedAt,
      );

    const current =
      totals.get(
        session.memberId,
      );

    if (
      current
    ) {

      current.seconds +=
        seconds;

      current.active =
        true;

      current.channelId =
        session.channelId;

      current.startedAt =
        session.startedAt;

      continue;
    }

    totals.set(
      session.memberId,
      {
        seconds,

        active:
          true,

        channelId:
          session.channelId,

        startedAt:
          session.startedAt,
      },
    );
  }

  /*
   * Membros.
   */

  const members =
    await db.member.findMany({
      where: {
        guildId:
          serverGuildId,
      },

      select: {
        id:
          true,

        username:
          true,

        displayName:
          true,
      },
    });

  const memberMap =
    new Map(
      members.map(
        (
          member,
        ) => [
          member.id,
          member,
        ],
      ),
    );

  /*
   * Ranking.
   */

  return [...totals.entries()]
    .map(
      (
        [
          memberId,
          data,
        ],
      ) => {

        const member =
          memberMap.get(
            memberId,
          );

        return {
          memberId,

          name:
            member?.displayName ||
            member?.username ||
            memberId,

          seconds:
            Math.max(
              0,
              Math.floor(
                data.seconds,
              ),
            ),

          active:
            data.active,

          channelId:
            data.channelId,

          startedAt:
            data.startedAt,
        };
      },
    )
    .sort(
      (
        a,
        b,
      ) => {

        if (
          b.seconds !==
          a.seconds
        ) {
          return (
            b.seconds -
            a.seconds
          );
        }

        if (
          a.active !==
          b.active
        ) {
          return a.active
            ? -1
            : 1;
        }

        return a.memberId.localeCompare(
          b.memberId,
        );
      },
    )
    .slice(
      0,
      Math.min(
        Math.max(
          1,
          Math.floor(
            Number(
              limit,
            ),
          ),
        ),
        100,
      ),
    );
}

/* =========================================================
   VOICE MEMBER
========================================================= */

async function getMemberVoiceSeconds(
  serverGuildId: string,
  userId: string,
): Promise<number> {

  const finished =
    await db.voiceSession.findMany({
      where: {
        guildId:
          serverGuildId,

        memberId:
          userId,

        endedAt: {
          not:
            null,
        },
      },

      select: {
        seconds:
          true,
      },
    });

  const totalFinished =
    finished.reduce(
      (
        total,
        session,
      ) =>
        total +
        Math.max(
          0,
          Number(
            session.seconds ??
              0,
          ),
        ),
      0,
    );

  const active =
    await db.voiceSession.findFirst({
      where: {
        guildId:
          serverGuildId,

        memberId:
          userId,

        endedAt:
          null,
      },

      orderBy: {
        startedAt:
          'desc',
      },

      select: {
        startedAt:
          true,
      },
    });

  const activeSeconds =
    active
      ? liveSeconds(
          active.startedAt,
        )
      : 0;

  return (
    totalFinished +
    activeSeconds
  );
}

/* =========================================================
   HEALTH
========================================================= */

app.get(
  '/api/health',
  async (
    _req: Request,
    res: Response,
  ) => {

    try {

      await db.$queryRaw`SELECT 1`;

      res.json({
        status:
          'online',

        database:
          'online',

        service:
          'Ghost Syndicate Web',

        timestamp:
          new Date().toISOString(),
      });

    } catch (
      error
    ) {

      console.error(
        '[WEB] Health:',
        error,
      );

      res
        .status(503)
        .json({
          status:
            'online',

          database:
            'offline',

          service:
            'Ghost Syndicate Web',

          timestamp:
            new Date().toISOString(),
        });

    }

  },
);

/* =========================================================
   OVERVIEW
========================================================= */

app.get(
  '/api/overview',
  async (
    _req: Request,
    res: Response,
  ) => {

    try {

      const id =
        getGuildId();

      const guild =
        await db.guild.findUnique({
          where: {
            id,
          },
        });

      const topVoice =
        await getTopVoice(
          id,
          10,
        );

      res.json({
        guild,

        topVoice:
          topVoice.map(
            (
              member,
            ) => ({
              memberId:
                member.memberId,

              name:
                member.name,

              seconds:
                member.seconds,

              active:
                member.active,

              channelId:
                member.channelId,

              duration:
                formatDuration(
                  member.seconds,
                ),
            }),
          ),
      });

    } catch (
      error
    ) {

      console.error(
        '[WEB API] /api/overview:',
        error,
      );

      res
        .status(500)
        .json({
          error:
            'Não foi possível carregar o painel.',
        });

    }

  },
);

/* =========================================================
   VOICE USER
========================================================= */

app.get(
  '/api/voice/:userId',
  async (
    req: Request,
    res: Response,
  ) => {

    try {

      const id =
        getGuildId();

      const userId =
        getParamString(
          req.params.userId,
        );

      if (
        !userId
      ) {

        res
          .status(400)
          .json({
            error:
              'ID do usuário inválido.',
          });

        return;

      }

      const seconds =
        await getMemberVoiceSeconds(
          id,
          userId,
        );

      res.json({
        userId,

        seconds,

        duration:
          formatDuration(
            seconds,
          ),

        clock:
          formatClock(
            seconds,
          ),
      });

    } catch (
      error
    ) {

      console.error(
        '[WEB API] /api/voice/:userId:',
        error,
      );

      res
        .status(500)
        .json({
          error:
            'Não foi possível carregar as horas em call.',
        });

    }

  },
);

/* =========================================================
   RANKING API
========================================================= */

app.get(
  '/api/ranking/voice',
  async (
    _req: Request,
    res: Response,
  ) => {

    try {

      const id =
        getGuildId();

      const ranking =
        await getTopVoice(
          id,
          25,
        );

      res.json({
        updatedAt:
          new Date().toISOString(),

        ranking:
          ranking.map(
            (
              member,
              index,
            ) => ({
              position:
                index + 1,

              memberId:
                member.memberId,

              name:
                member.name,

              seconds:
                member.seconds,

              duration:
                formatDuration(
                  member.seconds,
                ),

              clock:
                formatClock(
                  member.seconds,
                ),

              active:
                member.active,

              channelId:
                member.channelId,

              startedAt:
                member.startedAt
                  ?.toISOString() ??
                null,
            }),
          ),
      });

    } catch (
      error
    ) {

      console.error(
        '[WEB API] /api/ranking/voice:',
        error,
      );

      res
        .status(500)
        .json({
          error:
            'Não foi possível carregar o ranking.',
        });

    }

  },
);

/*
 * Alias usado pela página nova.
 */

app.get(
  '/api/ranking/live',
  async (
    _req: Request,
    res: Response,
  ) => {

    try {

      const id =
        getGuildId();

      const ranking =
        await getTopVoice(
          id,
          100,
        );

      res.json({
        updatedAt:
          new Date().toISOString(),

        ranking:
          ranking.map(
            (
              member,
              index,
            ) => ({
              position:
                index + 1,

              memberId:
                member.memberId,

              name:
                member.name,

              seconds:
                member.seconds,

              duration:
                formatDuration(
                  member.seconds,
                ),

              clock:
                formatClock(
                  member.seconds,
                ),

              active:
                member.active,

              channelId:
                member.channelId,

              startedAt:
                member.startedAt
                  ?.toISOString() ??
                null,
            }),
          ),
      });

    } catch (
      error
    ) {

      console.error(
        '[WEB API] /api/ranking/live:',
        error,
      );

      res
        .status(500)
        .json({
          error:
            'Não foi possível carregar o ranking.',
        });

    }

  },
);

/* =========================================================
   FINANCE
========================================================= */

app.get(
  '/api/finance',
  async (
    _req: Request,
    res: Response,
  ) => {

    try {

      const id =
        getGuildId();

      const guild =
        await db.guild.findUnique({
          where: {
            id,
          },
        });

      if (
        !guild
      ) {

        res
          .status(404)
          .json({
            error:
              'Servidor não encontrado no banco de dados.',
          });

        return;

      }

      const movements =
        await db.cashMovement.findMany({
          where: {
            guildId:
              id,
          },

          orderBy: {
            createdAt:
              'desc',
          },

          take:
            25,

          include: {
            member:
              true,
          },
        });

      res.json({
        balance:
          guild.cashBalance,

        balanceFormatted:
          formatMoney(
            guild.cashBalance,
          ),

        dailyGoal:
          guild.dailyGoal,

        reserve:
          guild.reserve,

        movements:
          movements.map(
            (
              movement,
            ) => ({
              id:
                movement.id,

              type:
                movement.type,

              amount:
                movement.amount,

              amountFormatted:
                formatMoney(
                  movement.amount,
                ),

              reason:
                movement.reason,

              responsible:
                movement.responsible,

              member:
                movement.member
                  ?.displayName ??
                null,

              createdAt:
                movement.createdAt,
            }),
          ),
      });

    } catch (
      error
    ) {

      console.error(
        '[WEB API] /api/finance:',
        error,
      );

      res
        .status(500)
        .json({
          error:
            'Não foi possível carregar os dados financeiros.',
        });

    }

  },
);

/* =========================================================
   OPERATIONS
========================================================= */

app.get(
  '/api/operations',
  async (
    _req: Request,
    res: Response,
  ) => {

    try {

      const id =
        getGuildId();

      const operations =
        await db.operation.findMany({
          where: {
            guildId:
              id,
          },

          orderBy: {
            createdAt:
              'desc',
          },

          take:
            50,
        });

      res.json({
        operations,
      });

    } catch (
      error
    ) {

      console.error(
        '[WEB API] /api/operations:',
        error,
      );

      res
        .status(500)
        .json({
          error:
            'Não foi possível carregar as operações.',
        });

    }

  },
);

/* =========================================================
   MISSIONS
========================================================= */

app.get(
  '/api/missions',
  async (
    _req: Request,
    res: Response,
  ) => {

    try {

      const id =
        getGuildId();

      const missions =
        await db.mission.findMany({
          where: {
            guildId:
              id,
          },

          orderBy: {
            createdAt:
              'desc',
          },
        });

      res.json({
        missions,
      });

    } catch (
      error
    ) {

      console.error(
        '[WEB API] /api/missions:',
        error,
      );

      res
        .status(500)
        .json({
          error:
            'Não foi possível carregar as missões.',
        });

    }

  },
);

/* =========================================================
   TRANSCRIPT HELPERS
========================================================= */

const TRANSCRIPT_DIRECTORY =
  path.resolve(
    process.cwd(),
    'storage',
    'transcripts',
  );

function isSafeTranscriptFilename(
  value: string,
): boolean {
  return (
    value.length > 0 &&
    value.length <= 255 &&
    value !== '.' &&
    value !== '..' &&
    !value.includes('\\') &&
    !value.includes('/') &&
    value.toLowerCase().endsWith('.html')
  );
}

function getTranscriptFilePath(
  transcriptIdOrFilename: string,
): string | null {
  const value =
    transcriptIdOrFilename.trim();

  if (!value) {
    return null;
  }

  /*
   * Compatibilidade:
   * /transcripts/1234567890123456789
   * -> transcript-1234567890123456789.html
   *
   * Também aceitamos o nome real do arquivo,
   * por exemplo:
   * recrutamento-usuario-123-2026-08-31.html
   */
  const filename =
    /^\d{15,25}$/.test(value)
      ? `transcript-${value}.html`
      : value;

  if (
    !isSafeTranscriptFilename(
      filename,
    )
  ) {
    return null;
  }

  const filePath =
    path.resolve(
      TRANSCRIPT_DIRECTORY,
      filename,
    );

  if (
    !filePath.startsWith(
      `${TRANSCRIPT_DIRECTORY}${path.sep}`,
    )
  ) {
    return null;
  }

  return filePath;
}

type TranscriptListItem = {
  filename: string;
  title: string;
  ticket: string;
  size: number;
  createdAt: string;
};

async function listTranscripts(): Promise<
  TranscriptListItem[]
> {
  let entries;

  try {
    entries =
      await readdir(
        TRANSCRIPT_DIRECTORY,
        {
          withFileTypes: true,
        },
      );
  } catch (
    error: unknown
  ) {
    const code =
      error &&
      typeof error === 'object' &&
      'code' in error
        ? String(
            (
              error as {
                code?: unknown;
              }
            ).code ?? '',
          )
        : '';

    if (
      code === 'ENOENT'
    ) {
      return [];
    }

    throw error;
  }

  const items:
    TranscriptListItem[] = [];

  for (
    const entry of entries
  ) {
    if (
      !entry.isFile() ||
      !isSafeTranscriptFilename(
        entry.name,
      )
    ) {
      continue;
    }

    const filePath =
      getTranscriptFilePath(
        entry.name,
      );

    if (!filePath) {
      continue;
    }

    try {
      const fileStat =
        await stat(
          filePath,
        );

      const filename =
        entry.name;

      const withoutExtension =
        filename.replace(
          /\.html$/i,
          '',
        );

      const ticket =
        withoutExtension
          .replace(
            /-\d{4}-\d{2}-\d{2}T.*$/i,
            '',
          )
          .replace(
            /^transcript-/i,
            '',
          );

      items.push({
        filename,

        title:
          ticket ||
          withoutExtension ||
          'Transcript',

        ticket:
          ticket ||
          withoutExtension ||
          'Transcript',

        size:
          fileStat.size,

        createdAt:
          fileStat.mtime.toISOString(),
      });
    } catch (
      error
    ) {
      console.error(
        '[WEB] Falha ao indexar transcript:',
        filePath,
        error,
      );
    }
  }

  return items.sort(
    (
      a,
      b,
    ) =>
      Date.parse(
        b.createdAt,
      ) -
      Date.parse(
        a.createdAt,
      ),
  );
}

/* =========================================================
   HISTÓRICO • API
========================================================= */

function getPositiveInt(
  value: unknown,
  fallback: number,
  min: number,
  max: number,
): number {
  const parsed = Number(value);

  if (!Number.isFinite(parsed)) {
    return fallback;
  }

  return Math.min(
    max,
    Math.max(
      min,
      Math.floor(parsed),
    ),
  );
}

function getOptionalDate(
  value: unknown,
): Date | null {
  if (
    typeof value !== 'string' ||
    !value.trim()
  ) {
    return null;
  }

  const parsed = new Date(value);

  return Number.isNaN(parsed.getTime())
    ? null
    : parsed;
}

function getHistorySearch(
  req: Request,
): string {
  return getParamString(
    req.query.q,
  )
    .trim()
    .toLowerCase();
}

function getHistoryKind(
  req: Request,
): string {
  return getParamString(
    req.query.kind,
  )
    .trim()
    .toUpperCase();
}

function getHistoryType(
  req: Request,
): string {
  return getParamString(
    req.query.type,
  )
    .trim()
    .toUpperCase();
}

function matchesHistorySearch(
  row: {
    member: string | null;
    reason: string;
    responsible: string | null;
    type: string;
    value?: string;
    status?: string;
  },
  search: string,
): boolean {
  if (!search) {
    return true;
  }

  return [
    row.member,
    row.reason,
    row.responsible,
    row.type,
    row.value,
    row.status,
  ]
    .filter(Boolean)
    .join(' ')
    .toLowerCase()
    .includes(search);
}

/* =========================================================
   HISTÓRICO • API
========================================================= */

app.get(
  '/api/history',
  async (
    req: Request,
    res: Response,
  ) => {
    try {
      const id = getGuildId();

      const search = getHistorySearch(req);
      const kind = getHistoryKind(req);
      const type = getHistoryType(req);
      const from = getOptionalDate(getParamString(req.query.from));
      const to = getOptionalDate(getParamString(req.query.to));
      const limit = getPositiveInt(
        getParamString(req.query.limit),
        100,
        1,
        250,
      );

      const [
        guild,
        movements,
        loans,
        transcripts,
      ] = await Promise.all([
        db.guild.findUnique({
          where: {
            id,
          },
          select: {
            cashBalance: true,
            dailyGoal: true,
            reserve: true,
          },
        }),

        db.cashMovement.findMany({
          where: {
            guildId: id,
          },
          orderBy: {
            createdAt: 'desc',
          },
          take: 250,
          include: {
            member: true,
          },
        }),

        db.loan.findMany({
          where: {
            guildId: id,
          },
          orderBy: {
            createdAt: 'desc',
          },
          take: 250,
          include: {
            member: true,
          },
        }),

        listTranscripts(),
      ]);

      const financeRecords = [
        ...movements.map(
          (movement) => ({
            id:
              `movement:${movement.id}`,

            kind:
              'movement' as const,

            type:
              movement.type,

            member:
              movement.member?.displayName ??
              null,

            memberId:
              movement.memberId,

            amount:
              Number(movement.amount),

            amountFormatted:
              formatMoney(
                Number(movement.amount),
              ),

            reason:
              movement.reason,

            responsible:
              movement.responsible,

            createdAt:
              movement.createdAt,
          }),
        ),

        ...loans.map(
          (loan) => ({
            id:
              `loan:${loan.id}`,

            kind:
              'loan' as const,

            type:
              loan.type,

            member:
              loan.member?.displayName ??
              loan.memberId,

            memberId:
              loan.memberId,

            value:
              loan.value,

            amountFormatted:
              loan.value,

            reason:
              `Empréstimo ${
                loan.type === 'DINHEIRO'
                  ? 'em dinheiro'
                  : 'de veículo'
              }`,

            responsible:
              null,

            interest:
              loan.interest,

            dueAt:
              loan.dueAt,

            status:
              loan.status,

            createdAt:
              loan.createdAt,
          }),
        ),
      ]
        .filter((row) => {
          const rowKind = row.kind.toUpperCase();
          const rowType = row.type.toUpperCase();

          if (
            kind &&
            kind !== 'ALL' &&
            rowKind !== kind
          ) {
            return false;
          }

          if (
            type &&
            type !== 'ALL'
          ) {
            if (type === 'LOAN') {
              if (row.kind !== 'loan') {
                return false;
              }
            } else if (
              row.kind !== 'movement' ||
              rowType !== type
            ) {
              return false;
            }
          }

          if (
            from &&
            row.createdAt.getTime() <
              from.getTime()
          ) {
            return false;
          }

          if (
            to &&
            row.createdAt.getTime() >
              to.getTime()
          ) {
            return false;
          }

          return matchesHistorySearch(
            row,
            search,
          );
        })
        .sort(
          (a, b) =>
            b.createdAt.getTime() -
            a.createdAt.getTime(),
        )
        .slice(0, limit);

      const totalIn = movements
        .filter(
          (movement) =>
            movement.type === 'IN',
        )
        .reduce(
          (sum, movement) =>
            sum + Number(movement.amount),
          0,
        );

      const totalOut = movements
        .filter(
          (movement) =>
            movement.type === 'OUT',
        )
        .reduce(
          (sum, movement) =>
            sum + Number(movement.amount),
          0,
        );

      const activeLoans = loans.filter(
        (loan) =>
          String(loan.status).toUpperCase() ===
          'ATIVO',
      ).length;

      const overdueLoans = loans.filter(
        (loan) =>
          String(loan.status).toUpperCase() ===
            'ATIVO' &&
          loan.dueAt.getTime() < Date.now(),
      ).length;

      const moneyLoans = loans.filter(
        (loan) =>
          loan.type === 'DINHEIRO',
      ).length;

      const vehicleLoans = loans.filter(
        (loan) =>
          loan.type === 'VEICULO',
      ).length;

      res
        .set(
          'Cache-Control',
          'private, no-store, max-age=0, must-revalidate',
        )
        .json({
          finance: financeRecords,
          transcripts,

          summary: {
            balance:
              guild?.cashBalance ?? 0,

            balanceFormatted:
              formatMoney(
                guild?.cashBalance ?? 0,
              ),

            dailyGoal:
              guild?.dailyGoal ?? 0,

            reserve:
              guild?.reserve ?? 0,

            totalIn,

            totalInFormatted:
              formatMoney(totalIn),

            totalOut,

            totalOutFormatted:
              formatMoney(totalOut),

            movementCount:
              movements.length,

            loanCount:
              loans.length,

            activeLoans,

            overdueLoans,

            moneyLoans,

            vehicleLoans,

            transcriptCount:
              transcripts.length,
          },

          filters: {
            q: search,
            kind: kind || 'ALL',
            type: type || 'ALL',
            from:
              from?.toISOString() ?? null,
            to:
              to?.toISOString() ?? null,
            limit,
          },

          updatedAt:
            new Date().toISOString(),
        });
    } catch (error) {
      console.error(
        '[WEB API] /api/history:',
        error,
      );

      res
        .status(500)
        .json({
          error:
            'Não foi possível carregar o histórico.',
        });
    }
  },
);

/* =========================================================
   TRANSCRIPTS • API
========================================================= */

app.get(
  '/api/transcripts',
  async (
    req: Request,
    res: Response,
  ) => {
    try {
      const search = getParamString(
        req.query.q,
      )
        .trim()
        .toLowerCase();

      const limit = getPositiveInt(
        getParamString(req.query.limit),
        100,
        1,
        250,
      );

      const transcripts =
        await listTranscripts();

      const filtered = transcripts
        .filter((item) => {
          if (!search) {
            return true;
          }

          return [
            item.filename,
            item.title,
            item.ticket,
          ]
            .join(' ')
            .toLowerCase()
            .includes(search);
        })
        .slice(0, limit)
        .map((item) => ({
          ...item,

          url:
            `/transcripts/${encodeURIComponent(
              item.filename,
            )}`,

          rawUrl:
            `/transcripts/raw/${encodeURIComponent(
              item.filename,
            )}`,

          sizeFormatted:
            `${(
              Number(item.size) / 1024
            ).toFixed(1)} KB`,
        }));

      res
        .set(
          'Cache-Control',
          'private, no-store, max-age=0, must-revalidate',
        )
        .json({
          transcripts: filtered,
          total: filtered.length,
          available:
            transcripts.length,
          updatedAt:
            new Date().toISOString(),
        });
    } catch (error) {
      console.error(
        '[WEB API] /api/transcripts:',
        error,
      );

      res
        .status(500)
        .json({
          error:
            'Não foi possível carregar os transcripts.',
        });
    }
  },
);

/* =========================================================
   HISTÓRICO • WEB
========================================================= */

app.get(
  '/historico',
  (
    _req: Request,
    res: Response,
  ) => {
    res
      .type('html')
      .send(
        createHistoryPage(),
      );
  },
);

/* =========================================================
   TRANSCRIPT VIEWER
========================================================= */

app.get(
  '/transcripts/:id',
  async (
    req: Request,
    res: Response,
  ) => {

    const transcriptId =
      decodeURIComponent(
        getParamString(
          req.params.id,
        ).trim(),
      );

    const filePath =
      getTranscriptFilePath(
        transcriptId,
      );

    if (
      !filePath
    ) {

      res
        .status(400)
        .type('html')
        .send(
          createTranscriptErrorPage(
            'Transcript inválido',
            'O identificador informado não possui um formato válido.',
          ),
        );

      return;

    }

    try {

      await readFile(
        filePath,
        'utf8',
      );

      res
        .type('html')
        .send(
          createTranscriptPage(
            transcriptId,
          ),
        );

    } catch (
      error
    ) {

      const code =
        error &&
        typeof error ===
          'object' &&
        'code' in error
          ? String(
              (
                error as {
                  code?: unknown;
                }
              ).code ??
                '',
            )
          : '';

      if (
        code ===
        'ENOENT'
      ) {

        res
          .status(404)
          .type('html')
          .send(
            createTranscriptErrorPage(
              'Transcript não encontrado',
              'O histórico ainda não foi arquivado ou não está disponível.',
            ),
          );

        return;

      }

      console.error(
        '[WEB] Transcript:',
        error,
      );

      res
        .status(500)
        .type('html')
        .send(
          createTranscriptErrorPage(
            'Erro ao carregar transcript',
            'O servidor encontrou um erro ao abrir esse histórico.',
          ),
        );

    }

  },
);

/* =========================================================
   TRANSCRIPT RAW
========================================================= */

app.get(
  '/transcripts/raw/:id',
  async (
    req: Request,
    res: Response,
  ) => {

    const transcriptId =
      decodeURIComponent(
        getParamString(
          req.params.id,
        ).trim(),
      );

    const filePath =
      getTranscriptFilePath(
        transcriptId,
      );

    if (
      !filePath
    ) {

      res
        .status(400)
        .type('text')
        .send(
          'Transcript inválido.',
        );

      return;

    }

    try {

      const html =
        await readFile(
          filePath,
          'utf8',
        );

      const theme =
        [
          '<style id="ghost-syndicate-theme">',

          ':root{',
          'color-scheme:dark!important;',
          '}',

          'html,body{',
          'margin:0!important;',
          'min-height:100%!important;',
          'background:#202225!important;',
          'color:#f2f3f5!important;',
          '}',

          'body{',
          'font-family:Inter,ui-sans-serif,system-ui,-apple-system,BlinkMacSystemFont,"Segoe UI",sans-serif!important;',
          'font-size:14px!important;',
          'line-height:1.55!important;',
          '}',

          '*,*::before,*::after{',
          'box-sizing:border-box!important;',
          'text-shadow:none!important;',
          'filter:none!important;',
          '}',

          'p,span,div,section,article,li,td,th,strong,b{',
          'color:#f2f3f5!important;',
          '}',

          'small,time,[class*="time"],[class*="Time"],[class*="timestamp"],[class*="Timestamp"]{',
          'color:#b5bac1!important;',
          '}',

          'a{',
          'color:#43ff98!important;',
          '}',

          'h1,h2,h3,h4,h5,h6{',
          'color:#ffffff!important;',
          '}',

          'pre,code{',
          'background:#111416!important;',
          'color:#e8fff0!important;',
          'border:1px solid rgba(67,255,152,.14)!important;',
          'border-radius:8px!important;',
          '}',

          '[class*="embed"],[class*="Embed"]{',
          'background:#2b2d31!important;',
          'color:#f2f3f5!important;',
          'border-color:rgba(67,255,152,.16)!important;',
          '}',

          'button,[role="button"]{',
          'display:none!important;',
          '}',

          'img{',
          'max-width:100%!important;',
          'height:auto!important;',
          'object-fit:contain!important;',
          '}',

          '::-webkit-scrollbar{',
          'width:10px;',
          'height:10px;',
          '}',

          '::-webkit-scrollbar-track{',
          'background:#18191c;',
          '}',

          '::-webkit-scrollbar-thumb{',
          'background:#365945;',
          'border-radius:999px;',
          '}',

          '::-webkit-scrollbar-thumb:hover{',
          'background:#43ff98;',
          '}',

          '</style>',
        ].join('');

      const finalHtml =
        html.includes(
          '</head>',
        )
          ? html.replace(
              '</head>',
              `${theme}</head>`,
            )
          : `${theme}${html}`;

      res
        .type('html')
        .set(
          'Cache-Control',
          'private, no-store, max-age=0, must-revalidate',
        )
        .send(
          finalHtml,
        );

    } catch (
      error
    ) {

      console.error(
        '[WEB] Transcript raw:',
        error,
      );

      res
        .status(404)
        .type('text')
        .send(
          'Transcript não encontrado.',
        );

    }

  },
);

/* =========================================================
   HISTÓRICO PAGE
========================================================= */

function createHistoryPage(): string {
  return `
<!doctype html>
<html lang="pt-BR">
<head>
  <meta charset="utf-8">

  <meta
    name="viewport"
    content="width=device-width, initial-scale=1"
  >

  <meta
    name="theme-color"
    content="#07100a"
  >

  <title>
    Histórico • Ghost Syndicate
  </title>

  <style>
    :root {
      color-scheme: dark;
      --bg: #050907;
      --panel: #0a120d;
      --panel-2: #0e1711;
      --border: rgba(67,255,152,.14);
      --border-strong: rgba(67,255,152,.32);
      --green: #43ff98;
      --text: #eef8f2;
      --muted: #789383;
      --purple: #7c5cff;
      --red: #f15b6b;
    }

    * {
      box-sizing: border-box;
    }

    html,
    body {
      margin: 0;
      min-height: 100%;
      background: var(--bg);
    }

    body {
      min-height: 100vh;
      color: var(--text);
      font-family:
        Inter,
        ui-sans-serif,
        system-ui,
        -apple-system,
        BlinkMacSystemFont,
        "Segoe UI",
        sans-serif;
      background:
        radial-gradient(
          circle at 12% 0%,
          rgba(67,255,152,.08),
          transparent 30%
        ),
        radial-gradient(
          circle at 100% 100%,
          rgba(124,92,255,.05),
          transparent 28%
        ),
        var(--bg);
    }

    main {
      width:
        min(
          1320px,
          calc(100% - 28px)
        );
      margin:
        0 auto;
      padding:
        18px 0 40px;
    }

    .topbar {
      display:
        flex;
      align-items:
        center;
      justify-content:
        space-between;
      gap:
        15px;
      margin-bottom:
        14px;
      padding:
        13px 15px;
      border:
        1px solid var(--border);
      border-radius:
        16px;
      background:
        rgba(8,14,10,.92);
      backdrop-filter:
        blur(16px);
      position:
        sticky;
      top:
        10px;
      z-index:
        10;
    }

    .brand {
      display:
        flex;
      align-items:
        center;
      gap:
        10px;
    }

    .logo {
      width:
        40px;
      height:
        40px;
      display:
        grid;
      place-items:
        center;
      border:
        1px solid var(--border-strong);
      border-radius:
        12px;
      background:
        #08130c;
      font-size:
        19px;
    }

    .eyebrow {
      margin:
        0 0 3px;
      color:
        var(--green);
      font-size:
        8px;
      font-weight:
        950;
      letter-spacing:
        .18em;
      text-transform:
        uppercase;
    }

    h1 {
      margin:
        0;
      font-size:
        18px;
      letter-spacing:
        -.03em;
    }

    .actions {
      display: flex;
      align-items: center;
      gap: 7px;
      flex-wrap: wrap;
    }

    .stats {
      display: grid;
      grid-template-columns: repeat(5, minmax(0, 1fr));
      gap: 9px;
      margin-bottom: 12px;
    }

    .stats article {
      min-width: 0;
      padding: 13px;
      border: 1px solid var(--border);
      border-radius: 14px;
      background: rgba(8,15,10,.86);
    }

    .stats span {
      display: block;
      margin-bottom: 7px;
      color: var(--muted);
      font-size: 8px;
      font-weight: 800;
    }

    .stats strong {
      display: block;
      overflow: hidden;
      color: var(--green);
      font-size: 14px;
      font-weight: 950;
      text-overflow: ellipsis;
      white-space: nowrap;
    }

    .status {
      padding:
        7px 10px;
      border:
        1px solid var(--border);
      border-radius:
        999px;
      color:
        #89a292;
      font-size:
        9px;
      font-weight:
        850;
      background:
        #09110c;
    }

    .hero {
      display:
        flex;
      align-items:
        flex-end;
      justify-content:
        space-between;
      gap:
        20px;
      padding:
        21px;
      margin-bottom:
        13px;
      border:
        1px solid var(--border);
      border-radius:
        18px;
      background:
        linear-gradient(
          135deg,
          rgba(12,27,17,.98),
          rgba(6,13,9,.98)
        );
    }

    .hero h2 {
      margin:
        0;
      font-size:
        clamp(
          30px,
          4vw,
          48px
        );
      line-height:
        .98;
      letter-spacing:
        -.055em;
    }

    .hero p {
      max-width:
        760px;
      margin:
        10px 0 0;
      color:
        var(--muted);
      line-height:
        1.6;
      font-size:
        12px;
    }

    .tabs {
      display:
        flex;
      gap:
        7px;
      flex-wrap:
        wrap;
      margin-bottom:
        10px;
    }

    .tab {
      border:
        1px solid var(--border);
      border-radius:
        10px;
      padding:
        8px 11px;
      background:
        #09110c;
      color:
        #9fb3a7;
      font:
        800 10px/1 inherit;
      cursor:
        pointer;
    }

    .tab.active {
      border-color:
        var(--border-strong);
      background:
        rgba(67,255,152,.08);
      color:
        var(--green);
    }

    .toolbar {
      display:
        flex;
      gap:
        7px;
      justify-content:
        space-between;
      align-items:
        center;
      margin:
        10px 12px;
      flex-wrap:
        wrap;
    }

    input,
    select {
      min-height:
        38px;
      padding:
        0 11px;
      border:
        1px solid var(--border);
      border-radius:
        10px;
      background:
        #08100b;
      color:
        var(--text);
      outline:
        none;
    }

    input {
      flex:
        1 1 280px;
    }

    .panel {
      overflow:
        hidden;
      border:
        1px solid var(--border);
      border-radius:
        18px;
      background:
        var(--panel);
      box-shadow:
        0 25px 80px rgba(0,0,0,.28);
    }

    .panel-head {
      display:
        flex;
      justify-content:
        space-between;
      align-items:
        center;
      gap:
        10px;
      padding:
        13px 15px;
      border-bottom:
        1px solid var(--border);
      background:
        var(--panel-2);
    }

    .panel-head strong {
      font-size:
        12px;
    }

    .count {
      color:
        #657a6d;
      font-size:
        9px;
    }

    .list {
      display:
        grid;
    }

    .row {
      display:
        grid;
      grid-template-columns:
        125px
        minmax(0,1fr)
        auto
        auto;
      gap:
        14px;
      align-items:
        center;
      padding:
        14px 15px;
      border-bottom:
        1px solid rgba(255,255,255,.035);
      text-decoration:
        none;
      transition:
        .16s ease;
    }

    .row:last-child {
      border-bottom:
        0;
    }

    a.row:hover {
      background:
        rgba(67,255,152,.035);
      transform:
        translateX(2px);
    }

    .tag {
      width:
        fit-content;
      padding:
        5px 7px;
      border:
        1px solid var(--border);
      border-radius:
        7px;
      background:
        #09120c;
      color:
        var(--green);
      font-size:
        8px;
      font-weight:
        900;
      letter-spacing:
        .08em;
      text-transform:
        uppercase;
    }

    .tag.loan {
      color:
        #bdafff;
      border-color:
        rgba(124,92,255,.22);
      background:
        rgba(124,92,255,.07);
    }

    .tag.out {
      color:
        #ffb6be;
      border-color:
        rgba(241,91,107,.22);
      background:
        rgba(241,91,107,.06);
    }

    .title {
      min-width:
        0;
      font-size:
        11px;
      font-weight:
        850;
      overflow:
        hidden;
      text-overflow:
        ellipsis;
      white-space:
        nowrap;
    }

    .meta {
      margin-top:
        3px;
      color:
        #697c70;
      font-size:
        9px;
      overflow:
        hidden;
      text-overflow:
        ellipsis;
      white-space:
        nowrap;
    }

    .value {
      text-align:
        right;
      white-space:
        nowrap;
      font-size:
        11px;
      font-weight:
        850;
    }

    .date {
      color:
        #718478;
      white-space:
        nowrap;
      font-size:
        9px;
    }

    .arrow {
      color:
        #597063;
      font-size:
        13px;
    }

    .empty,
    .loading,
    .error {
      padding:
        28px 16px;
      color:
        #718478;
      text-align:
        center;
      font-size:
        11px;
    }

    .error {
      color:
        #e9a0aa;
    }

    .hidden {
      display:
        none !important;
    }

    @media (
      max-width: 850px
    ) {
      .stats {
        grid-template-columns: repeat(2, minmax(0, 1fr));
      }

      .row {
        grid-template-columns:
          92px
          minmax(0,1fr)
          auto;
      }

      .row .date {
        display:
          none;
      }
    }

    @media (
      max-width: 560px
    ) {
      main {
        width:
          min(
            100% - 14px,
            700px
          );
      }

      .topbar,
      .hero {
        align-items:
          flex-start;
        flex-direction:
          column;
      }

      .row {
        grid-template-columns:
          80px
          minmax(0,1fr)
          auto;
        gap:
          9px;
        padding:
          12px;
      }

      .value {
        display:
          none;
      }
    }
  </style>
</head>

<body>
  <main>
    <header class="topbar">
      <div class="brand">
        <div class="logo">
          👻
        </div>

        <div>
          <p class="eyebrow">
            Ghost Syndicate
          </p>

          <h1>
            Central de Histórico
          </h1>
        </div>
      </div>

      <div class="actions">
        <div class="status" id="statusBadge">
          🟢 Registros preservados
        </div>
        <button class="tab" id="refreshHistory" type="button">
          ↻ Atualizar
        </button>
      </div>
    </header>

    <section class="hero">
      <div>
        <h2>
          Histórico completo.
        </h2>

        <p>
          Financeiro e atendimentos arquivados
          em um único lugar. Clique em um
          transcript para abrir a conversa completa.
        </p>
      </div>

      <div class="tag">
        ADMIN • DONO
      </div>
    </section>

    <section class="stats" id="stats">
      <article><span>🏦 Saldo atual</span><strong id="statBalance">—</strong></article>
      <article><span>📥 Entradas</span><strong id="statIn">—</strong></article>
      <article><span>📤 Saídas</span><strong id="statOut">—</strong></article>
      <article><span>💳 Empréstimos ativos</span><strong id="statLoans">—</strong></article>
      <article><span>📜 Transcripts</span><strong id="statTranscripts">—</strong></article>
    </section>

    <div class="tabs">
      <button
        class="tab active"
        data-tab="finance"
        type="button"
      >
        💰 Financeiro
      </button>

      <button
        class="tab"
        data-tab="transcripts"
        type="button"
      >
        📜 Transcripts
      </button>
    </div>

    <section
      id="financeSection"
      class="panel"
    >
      <header class="panel-head">
        <strong>
          💰 Histórico financeiro
        </strong>

        <span
          id="financeCount"
          class="count"
        >
          —
        </span>
      </header>

      <div class="toolbar">
        <input
          id="financeSearch"
          type="search"
          placeholder="Pesquisar membro, motivo ou responsável..."
        >

        <select id="financeFilter">
          <option value="all">
            Tudo
          </option>

          <option value="IN">
            📥 Entradas
          </option>

          <option value="OUT">
            📤 Saídas
          </option>

          <option value="LOAN">
            💳 Empréstimos
          </option>
        </select>
      </div>

      <div
        id="financeList"
        class="list"
      >
        <div class="loading">
          Carregando histórico financeiro...
        </div>
      </div>
    </section>

    <section
      id="transcriptsSection"
      class="panel hidden"
    >
      <header class="panel-head">
        <strong>
          📜 Transcripts arquivados
        </strong>

        <span
          id="transcriptCount"
          class="count"
        >
          —
        </span>
      </header>

      <div class="toolbar">
        <input
          id="transcriptSearch"
          type="search"
          placeholder="Pesquisar ticket ou arquivo..."
        >
      </div>

      <div
        id="transcriptList"
        class="list"
      >
        <div class="loading">
          Carregando transcripts...
        </div>
      </div>
    </section>
  </main>

  <script>
    const state = {
      finance: [],
      transcripts: [],
    };

    const escapeHtml = (
      value
    ) =>
      String(
        value ?? ''
      )
        .replaceAll(
          '&',
          '&amp;'
        )
        .replaceAll(
          '<',
          '&lt;'
        )
        .replaceAll(
          '>',
          '&gt;'
        )
        .replaceAll(
          '"',
          '&quot;'
        )
        .replaceAll(
          "'",
          '&#039;'
        );

    const date = (
      value
    ) => {
      const parsed =
        new Date(
          value
        );

      if (
        Number.isNaN(
          parsed.getTime()
        )
      ) {
        return '—';
      }

      return new Intl.DateTimeFormat(
        'pt-BR',
        {
          dateStyle:
            'short',
          timeStyle:
            'short',
        }
      ).format(
        parsed
      );
    };

    const money = (
      value
    ) => {
      const number =
        Number(
          value
        );

      if (
        !Number.isFinite(
          number
        )
      ) {
        return '—';
      }

      return number.toLocaleString(
        'pt-BR',
        {
          style:
            'currency',
          currency:
            'BRL',
          maximumFractionDigits:
            0,
        }
      );
    };

    function renderFinance() {
      const root =
        document.getElementById(
          'financeList'
        );

      const query =
        document
          .getElementById(
            'financeSearch'
          )
          .value
          .trim()
          .toLowerCase();

      const filter =
        document
          .getElementById(
            'financeFilter'
          )
          .value;

      const rows =
        state.finance.filter(
          (
            item
          ) => {
            const haystack =
              [
                item.member,
                item.reason,
                item.responsible,
                item.type,
                item.interest,
                item.status,
                item.value,
              ]
                .filter(
                  Boolean
                )
                .join(' ')
                .toLowerCase();

            const matchesSearch =
              !query ||
              haystack.includes(
                query
              );

            const matchesFilter =
              filter ===
                'all' ||
              (
                filter ===
                  'LOAN' &&
                item.kind ===
                  'loan'
              ) ||
              (
                filter !==
                  'LOAN' &&
                item.kind ===
                  'movement' &&
                item.type ===
                  filter
              );

            return (
              matchesSearch &&
              matchesFilter
            );
          }
        );

      document.getElementById(
        'financeCount'
      ).textContent =
        rows.length +
        ' registro(s)';

      if (
        !rows.length
      ) {
        root.innerHTML =
          '<div class="empty">' +
          'Nenhum registro encontrado.' +
          '</div>';

        return;
      }

      root.innerHTML =
        rows
          .map(
            (
              item
            ) => {
              const isLoan =
                item.kind ===
                'loan';

              const isOut =
                item.type ===
                'OUT';

              const tagClass =
                isLoan
                  ? 'loan'
                  : isOut
                    ? 'out'
                    : '';

              const tag =
                isLoan
                  ? '💳 Empréstimo'
                  : isOut
                    ? '📤 Saída'
                    : '📥 Entrada';

              const title =
                escapeHtml(
                  item.member ||
                  'Sem membro'
                );

              const meta =
                escapeHtml(
                  [
                    item.reason,
                    item.responsible
                      ? 'por ' +
                        item.responsible
                      : null,
                    isLoan &&
                    item.interest
                      ? 'juros: ' +
                        item.interest
                      : null,
                    isLoan &&
                    item.dueAt
                      ? 'vence: ' +
                        date(
                          item.dueAt
                        )
                      : null,
                  ]
                    .filter(
                      Boolean
                    )
                    .join(
                      ' • '
                    )
                );

              const formattedValue =
                isLoan
                  ? (
                      item.amountFormatted ||
                      item.value ||
                      '—'
                    )
                  : (
                      (
                        isOut
                          ? '- '
                          : '+ '
                      ) +
                      money(
                        item.amount
                      )
                    );

              return (
                '<article class="row">' +
                  '<div>' +
                    '<div class="tag ' +
                      tagClass +
                    '">' +
                      tag +
                    '</div>' +
                  '</div>' +

                  '<div>' +
                    '<div class="title">' +
                      title +
                    '</div>' +
                    '<div class="meta">' +
                      meta +
                    '</div>' +
                  '</div>' +

                  '<div class="value">' +
                    escapeHtml(
                      formattedValue
                    ) +
                  '</div>' +

                  '<div class="date">' +
                    escapeHtml(
                      date(
                        item.createdAt
                      )
                    ) +
                  '</div>' +
                '</article>'
              );
            }
          )
          .join('');
    }

    function renderTranscripts() {
      const root =
        document.getElementById(
          'transcriptList'
        );

      const query =
        document
          .getElementById(
            'transcriptSearch'
          )
          .value
          .trim()
          .toLowerCase();

      const rows =
        state.transcripts.filter(
          (
            item
          ) =>
            !query ||
            [
              item.filename,
              item.title,
              item.ticket,
            ]
              .join(' ')
              .toLowerCase()
              .includes(
                query
              )
        );

      document.getElementById(
        'transcriptCount'
      ).textContent =
        rows.length +
        ' transcript(s)';

      if (
        !rows.length
      ) {
        root.innerHTML =
          '<div class="empty">' +
          'Nenhum transcript encontrado.' +
          '</div>';

        return;
      }

      root.innerHTML =
        rows
          .map(
            (
              item
            ) => {
              const url =
                '/transcripts/' +
                encodeURIComponent(
                  item.filename
                );

              return (
                '<a class="row" href="' +
                  url +
                '">' +

                  '<div>' +
                    '<div class="tag">' +
                      '📜 Transcript' +
                    '</div>' +
                  '</div>' +

                  '<div>' +
                    '<div class="title">' +
                      escapeHtml(
                        item.title
                      ) +
                    '</div>' +

                    '<div class="meta">' +
                      escapeHtml(
                        item.filename
                      ) +
                      ' • ' +
                      escapeHtml(
                        (
                          Number(
                            item.size ||
                            0
                          ) /
                          1024
                        ).toFixed(1)
                      ) +
                      ' KB' +
                    '</div>' +
                  '</div>' +

                  '<div class="date">' +
                    escapeHtml(
                      date(
                        item.createdAt
                      )
                    ) +
                  '</div>' +

                  '<div class="arrow">' +
                    '→' +
                  '</div>' +

                '</a>'
              );
            }
          )
          .join('');
    }

    async function loadHistory() {
      try {
        const response =
          await fetch(
            '/api/history',
            {
              cache:
                'no-store',
            }
          );

        if (
          !response.ok
        ) {
          throw new Error(
            'Falha ao carregar histórico.'
          );
        }

        const data =
          await response.json();

        state.finance =
          Array.isArray(
            data.finance
          )
            ? data.finance
            : [];

        state.transcripts =
          Array.isArray(
            data.transcripts
          )
            ? data.transcripts
            : [];

        const summary =
          data.summary || {};

        document.getElementById(
          'statBalance'
        ).textContent =
          summary.balanceFormatted ||
          'R$ 0';

        document.getElementById(
          'statIn'
        ).textContent =
          summary.totalInFormatted ||
          'R$ 0';

        document.getElementById(
          'statOut'
        ).textContent =
          summary.totalOutFormatted ||
          'R$ 0';

        document.getElementById(
          'statLoans'
        ).textContent =
          String(
            summary.activeLoans ||
            0
          );

        document.getElementById(
          'statTranscripts'
        ).textContent =
          String(
            summary.transcriptCount ||
            0
          );

        const status =
          document.getElementById(
            'statusBadge'
          );

        if (status) {
          status.textContent =
            '🟢 Atualizado agora';
        }

        renderFinance();
        renderTranscripts();

      } catch (
        error
      ) {
        console.error(
          '[HISTORY PAGE]',
          error
        );

        document.getElementById(
          'financeList'
        ).innerHTML =
          '<div class="error">' +
          'Não foi possível carregar o histórico.' +
          '</div>';

        document.getElementById(
          'transcriptList'
        ).innerHTML =
          '<div class="error">' +
          'Não foi possível carregar os transcripts.' +
          '</div>';
      }
    }

    document.querySelectorAll(
      '.tab'
    ).forEach(
      (
        tab
      ) => {
        tab.addEventListener(
          'click',
          () => {
            document.querySelectorAll(
              '.tab'
            ).forEach(
              (
                other
              ) =>
                other.classList.remove(
                  'active'
                )
            );

            tab.classList.add(
              'active'
            );

            const isFinance =
              tab.dataset.tab ===
              'finance';

            document
              .getElementById(
                'financeSection'
              )
              .classList.toggle(
                'hidden',
                !isFinance
              );

            document
              .getElementById(
                'transcriptsSection'
              )
              .classList.toggle(
                'hidden',
                isFinance
              );
          }
        );
      }
    );

    document.getElementById(
      'financeSearch'
    ).addEventListener(
      'input',
      renderFinance
    );

    document.getElementById(
      'financeFilter'
    ).addEventListener(
      'change',
      renderFinance
    );

    document.getElementById(
      'transcriptSearch'
    ).addEventListener(
      'input',
      renderTranscripts
    );

    loadHistory();
  </script>
</body>
</html>
`;
}

/* =========================================================
   TRANSCRIPT ERROR PAGE
========================================================= */

function createTranscriptErrorPage(
  title: string,
  message: string,
): string {

  const safeTitle =
    escapeHtml(
      title,
    );

  const safeMessage =
    escapeHtml(
      message,
    );

  return `
<!doctype html>
<html lang="pt-BR">
<head>
  <meta charset="utf-8">
  <meta
    name="viewport"
    content="width=device-width, initial-scale=1"
  >
  <meta
    name="theme-color"
    content="#07100a"
  >
  <meta
    name="color-scheme"
    content="dark"
  >
  <title>${safeTitle} • Ghost Syndicate</title>
  <style>
    :root {
      color-scheme: dark;
      --bg: #030806;
      --panel: #07100a;
      --green: #43ff98;
      --text: #ecfff4;
      --muted: #789383;
      --border: rgba(67, 255, 152, .16);
    }

    * { box-sizing: border-box; }

    html, body {
      margin: 0;
      min-height: 100%;
      background: var(--bg);
    }

    body {
      min-height: 100vh;
      display: grid;
      place-items: center;
      padding: 24px;
      color: var(--text);
      font-family: Inter, ui-sans-serif, system-ui, -apple-system, BlinkMacSystemFont, "Segoe UI", sans-serif;
      background:
        radial-gradient(circle at 20% 10%, rgba(67, 255, 152, .08), transparent 35%),
        radial-gradient(circle at 80% 90%, rgba(67, 255, 152, .04), transparent 30%),
        var(--bg);
    }

    main {
      width: min(620px, 100%);
      padding: 30px;
      border: 1px solid var(--border);
      border-radius: 20px;
      background: rgba(7, 16, 10, .96);
      box-shadow: 0 30px 80px rgba(0, 0, 0, .45);
    }

    .brand {
      color: var(--green);
      font-size: 10px;
      font-weight: 900;
      letter-spacing: .18em;
      text-transform: uppercase;
    }

    h1 {
      margin: 10px 0 8px;
      font-size: clamp(28px, 6vw, 46px);
      line-height: 1;
    }

    p {
      margin: 0;
      color: var(--muted);
      line-height: 1.7;
      font-size: 14px;
    }
  </style>
</head>
<body>
  <main>
    <div class="brand">GHOST SYNDICATE • TRANSCRIPT</div>
    <h1>${safeTitle}</h1>
    <p>${safeMessage}</p>
  </main>
</body>
</html>
`;
}

/* =========================================================
   TRANSCRIPT PAGE
========================================================= */

function createTranscriptPage(
  transcriptId: string,
): string {

  const safeId =
    escapeHtml(
      transcriptId,
    );

  const rawUrl =
    `/transcripts/raw/${encodeURIComponent(
      transcriptId,
    )}`;

  return `
<!doctype html>

<html lang="pt-BR">

<head>

  <meta charset="utf-8">

  <meta
    name="viewport"
    content="width=device-width, initial-scale=1"
  >

  <meta
    name="theme-color"
    content="#07100a"
  >

  <meta
    name="color-scheme"
    content="dark"
  >

  <title>
    Transcript #${safeId} • Ghost Syndicate
  </title>

  <style>

    :root {

      color-scheme:
        dark;

      --bg:
        #060a08;

      --panel:
        #0b110e;

      --panel-2:
        #101713;

      --border:
        rgba(
          67,
          255,
          152,
          .13
        );

      --border-strong:
        rgba(
          67,
          255,
          152,
          .30
        );

      --green:
        #43ff98;

      --green-soft:
        #b0ffd0;

      --text:
        #eef8f2;

      --muted:
        #7c9484;

    }

    * {
      box-sizing:
        border-box;
    }

    html,
    body {
      margin:
        0;

      min-height:
        100%;

      background:
        var(--bg);
    }

    body {

      min-height:
        100vh;

      overflow-x:
        hidden;

      color:
        var(--text);

      font-family:
        Inter,
        ui-sans-serif,
        system-ui,
        -apple-system,
        BlinkMacSystemFont,
        "Segoe UI",
        sans-serif;

      background:
        radial-gradient(
          circle at 10% 0%,
          rgba(
            67,
            255,
            152,
            .08
          ),
          transparent 28%
        ),
        radial-gradient(
          circle at 100% 100%,
          rgba(
            67,
            255,
            152,
            .035
          ),
          transparent 25%
        ),
        var(--bg);

    }

    body::before {

      content:
        "";

      position:
        fixed;

      inset:
        0;

      pointer-events:
        none;

      opacity:
        .010;

      background:
        repeating-linear-gradient(
          180deg,
          rgba(
            255,
            255,
            255,
            .22
          ) 0,
          rgba(
            255,
            255,
            255,
            .22
          ) 1px,
          transparent 1px,
          transparent 8px
        );

    }

    body::after {

      content:
        "";

      position:
        fixed;

      width:
        420px;

      height:
        420px;

      left:
        -220px;

      top:
        -220px;

      pointer-events:
        none;

      border-radius:
        50%;

      background:
        radial-gradient(
          circle,
          rgba(
            67,
            255,
            152,
            .09
          ),
          transparent 68%
        );

      filter:
        blur(
          30px
        );

      animation:
        glow
        12s
        ease-in-out
        infinite;

    }

    @keyframes glow {

      0%,
      100% {
        transform:
          translate(
            0,
            0
          );
      }

      50% {
        transform:
          translate(
            90px,
            50px
          );
      }

    }

    @keyframes fade {

      from {
        opacity:
          0;

        transform:
          translateY(
            9px
          );
      }

      to {
        opacity:
          1;

        transform:
          translateY(
            0
          );
      }

    }

    .page {

      position:
        relative;

      z-index:
        1;

      width:
        min(
          1500px,
          calc(
            100% - 28px
          )
        );

      margin:
        0 auto;

      padding:
        14px 0 24px;

    }

    .topbar {

      position:
        sticky;

      top:
        10px;

      z-index:
        30;

      display:
        flex;

      align-items:
        center;

      justify-content:
        space-between;

      gap:
        12px;

      min-height:
        58px;

      padding:
        9px 12px;

      margin-bottom:
        12px;

      border:
        1px solid
        var(--border);

      border-radius:
        16px;

      background:
        rgba(
          8,
          14,
          10,
          .94
        );

      backdrop-filter:
        blur(
          14px
        );

      animation:
        fade
        .45s
        ease
        both;

    }

    .brand {

      display:
        flex;

      align-items:
        center;

      gap:
        10px;

    }

    .logo {

      width:
        40px;

      height:
        40px;

      display:
        grid;

      place-items:
        center;

      border:
        1px solid
        var(--border-strong);

      border-radius:
        12px;

      background:
        linear-gradient(
          145deg,
          #10271a,
          #07110b
        );

      color:
        var(--green);

    }

    .eyebrow {

      margin:
        0 0 1px;

      color:
        var(--green);

      font-size:
        8px;

      font-weight:
        950;

      letter-spacing:
        .19em;

      text-transform:
        uppercase;

    }

    .brand-title {

      margin:
        0;

      font-size:
        13px;

      font-weight:
        850;

    }

    .actions {

      display:
        flex;

      align-items:
        center;

      gap:
        7px;

      flex-wrap:
        wrap;

    }

    .status {

      display:
        inline-flex;

      align-items:
        center;

      gap:
        7px;

      min-height:
        31px;

      padding:
        7px 10px;

      border:
        1px solid
        var(--border);

      border-radius:
        999px;

      background:
        #09110c;

      color:
        #87a08f;

      font-size:
        9px;

      font-weight:
        800;

    }

    .dot {

      width:
        7px;

      height:
        7px;

      border-radius:
        50%;

      background:
        var(--green);

      box-shadow:
        0 0 12px
        rgba(
          67,
          255,
          152,
          .85
        );

    }

    .button {

      display:
        inline-flex;

      align-items:
        center;

      justify-content:
        center;

      gap:
        6px;

      min-height:
        31px;

      padding:
        7px 10px;

      border:
        1px solid
        var(--border);

      border-radius:
        9px;

      background:
        #09110c;

      color:
        var(--green-soft);

      font:
        800 9px/1
        inherit;

      text-decoration:
        none;

      cursor:
        pointer;

      transition:
        .18s
        ease;

    }

    .button:hover {

      border-color:
        var(--border-strong);

      background:
        #0e1912;

      transform:
        translateY(
          -1px
        );

    }

    .hero {

      display:
        grid;

      grid-template-columns:
        minmax(
          0,
          1fr
        )
        250px;

      gap:
        12px;

      margin-bottom:
        12px;

    }

    .hero-main,
    .hero-id {

      border:
        1px solid
        var(--border);

      border-radius:
        17px;

      background:
        linear-gradient(
          135deg,
          rgba(
            11,
            24,
            15,
            .97
          ),
          rgba(
            6,
            13,
            9,
            .97
          )
        );

      animation:
        fade
        .5s
        .05s
        ease
        both;

    }

    .hero-main {

      padding:
        22px;

    }

    .hero-kicker {

      margin:
        0 0 7px;

      color:
        var(--green);

      font-size:
        9px;

      font-weight:
        950;

      letter-spacing:
        .17em;

      text-transform:
        uppercase;

    }

    .hero-main h1 {

      margin:
        0;

      font-size:
        clamp(
          28px,
          4vw,
          48px
        );

      line-height:
        .98;

      letter-spacing:
        -.055em;

    }

    .hero-main p {

      max-width:
        760px;

      margin:
        11px 0 0;

      color:
        var(--muted);

      font-size:
        12px;

      line-height:
        1.6;

    }

    .hero-id {

      display:
        flex;

      flex-direction:
        column;

      justify-content:
        space-between;

      padding:
        17px;

      animation-delay:
        .08s;

    }

    .label {

      margin:
        0 0 7px;

      color:
        #587060;

      font-size:
        8px;

      font-weight:
        950;

      letter-spacing:
        .14em;

      text-transform:
        uppercase;

    }

    .id {

      margin:
        0;

      color:
        var(--green-soft);

      font:
        800 11px/1.4
        ui-monospace,
        monospace;

      overflow-wrap:
        anywhere;

    }

    .archive {

      display:
        flex;

      align-items:
        center;

      gap:
        8px;

      margin-top:
        14px;

      padding:
        9px 10px;

      border:
        1px solid
        var(--border);

      border-radius:
        10px;

      background:
        rgba(
          67,
          255,
          152,
          .025
        );

      color:
        #88a997;

      font-size:
        9px;

    }

    .content {

      overflow:
        hidden;

      border:
        1px solid
        var(--border);

      border-radius:
        17px;

      background:
        var(--panel);

      box-shadow:
        0 25px 90px
        rgba(
          0,
          0,
          0,
          .28
        );

      animation:
        fade
        .55s
        .1s
        ease
        both;

    }

    .content-head {

      display:
        flex;

      align-items:
        center;

      justify-content:
        space-between;

      gap:
        12px;

      padding:
        11px 13px;

      border-bottom:
        1px solid
        var(--border);

      background:
        var(--panel-2);

    }

    .content-title {

      display:
        flex;

      align-items:
        center;

      gap:
        7px;

      margin:
        0;

      font-size:
        11px;

      font-weight:
        900;

    }

    .content-title span {

      color:
        var(--green);

    }

    .tools {

      display:
        flex;

      gap:
        6px;

      flex-wrap:
        wrap;

    }

    .conversation {

      padding:
        8px;

      background:
        #080d0a;

    }

    .window {

      overflow:
        hidden;

      border:
        1px solid
        rgba(
          67,
          255,
          152,
          .10
        );

      border-radius:
        12px;

      background:
        #202225;

    }

    .channel {

      display:
        flex;

      align-items:
        center;

      gap:
        8px;

      min-height:
        37px;

      padding:
        8px 11px;

      border-bottom:
        1px solid
        rgba(
          255,
          255,
          255,
          .06
        );

      background:
        #1b1d20;

      color:
        #b5bac1;

      font-size:
        10px;

    }

    .channel-symbol {

      color:
        #8f979f;

      font-size:
        15px;

    }

    .channel-name {

      color:
        #f2f3f5;

      font-weight:
        850;

    }

    .channel-id {

      overflow:
        hidden;

      text-overflow:
        ellipsis;

      white-space:
        nowrap;

      color:
        #6c737b;

      font-size:
        8px;

    }

    .frame {

      display:
        block;

      width:
        100%;

      height:
        74vh;

      min-height:
        650px;

      border:
        0;

      background:
        #202225;

    }

    .window-footer {

      padding:
        8px 10px;

      border-top:
        1px solid
        rgba(
          255,
          255,
          255,
          .05
        );

      background:
        #1b1d20;

      color:
        #6e757d;

      font-size:
        8px;

    }

    .footer {

      display:
        flex;

      justify-content:
        space-between;

      gap:
        10px;

      margin-top:
        9px;

      color:
        #50675a;

      font-size:
        8px;

      flex-wrap:
        wrap;

    }

    .footer strong {

      color:
        #79a488;

    }

    @media (
      max-width: 850px
    ) {

      .hero {

        grid-template-columns:
          1fr;

      }

    }

    @media (
      max-width: 620px
    ) {

      .page {

        width:
          calc(
            100% - 12px
          );

        padding-top:
          6px;

      }

      .topbar {

        position:
          relative;

        top:
          auto;

        align-items:
          flex-start;

        flex-direction:
          column;

      }

      .content-head {

        align-items:
          flex-start;

        flex-direction:
          column;

      }

      .tools {

        justify-content:
          flex-start;

      }

      .conversation {

        padding:
          5px;

      }

      .frame {

        min-height:
          500px;

        height:
          72vh;

      }

    }

  </style>

</head>

<body>

  <main class="page">

    <header class="topbar">

      <div class="brand">

        <div class="logo">
          👻
        </div>

        <div>

          <p class="eyebrow">
            GHOST SYNDICATE
          </p>

          <p class="brand-title">
            Central de Transcripts
          </p>

        </div>

      </div>

      <div class="actions">

        <div class="status">

          <span class="dot"></span>

          Histórico arquivado

        </div>

        <button
          id="copy"
          class="button"
          type="button"
        >
          ⧉ Copiar ID
        </button>

      </div>

    </header>

    <section class="hero">

      <div class="hero-main">

        <p class="hero-kicker">
          Transcript oficial
        </p>

        <h1>
          Histórico do atendimento.
        </h1>

        <p>
          Registro preservado da conversa
          realizada pela Central de Atendimento
          da Ghost Syndicate.
        </p>

      </div>

      <aside class="hero-id">

        <div>

          <p class="label">
            Identificador
          </p>

          <p class="id">
            #${safeId}
          </p>

        </div>

        <div class="archive">

          <span>
            🟢
          </span>

          <span>
            Histórico disponível para consulta.
          </span>

        </div>

      </aside>

    </section>

    <section class="content">

      <header class="content-head">

        <h2 class="content-title">

          <span>
            💬
          </span>

          Conversa do atendimento

        </h2>

        <div class="tools">

          <a
            class="button"
            href="${rawUrl}"
            target="_blank"
            rel="noopener noreferrer"
          >
            ↗ Original
          </a>

          <button
            id="fullscreen"
            class="button"
            type="button"
          >
            ⛶ Tela cheia
          </button>

        </div>

      </header>

      <div class="conversation">

        <div
          id="viewer"
          class="window"
        >

          <div class="channel">

            <span class="channel-symbol">
              #
            </span>

            <span class="channel-name">
              Transcript
            </span>

            <span class="channel-id">
              #${safeId}
            </span>

          </div>

          <iframe
            id="frame"
            class="frame"
            src="${rawUrl}"
            title="Transcript Ghost Syndicate"
            loading="eager"
            referrerpolicy="no-referrer"
          ></iframe>

          <div class="window-footer">

            📜 Histórico preservado da
            Central de Atendimento
            • Ghost Syndicate

          </div>

        </div>

      </div>

    </section>

    <footer class="footer">

      <span>

        Ghost Syndicate
        •
        <strong>Organização</strong>
        •
        <strong>Lealdade</strong>
        •
        <strong>Resultado</strong>

      </span>

      <span>
        Transcript oficial
      </span>

    </footer>

  </main>

  <script>

    const transcriptId =
      ${JSON.stringify(
        transcriptId,
      )};

    const copyButton =
      document.getElementById(
        'copy',
      );

    const fullscreenButton =
      document.getElementById(
        'fullscreen',
      );

    const viewer =
      document.getElementById(
        'viewer',
      );

    copyButton.addEventListener(
      'click',
      async () => {

        try {

          await navigator.clipboard.writeText(
            transcriptId,
          );

          const oldText =
            copyButton.textContent;

          copyButton.textContent =
            '✓ Copiado';

          setTimeout(
            () => {

              copyButton.textContent =
                oldText;

            },
            1700,
          );

        } catch {

          copyButton.textContent =
            '✕ Falhou';

          setTimeout(
            () => {

              copyButton.textContent =
                '⧉ Copiar ID';

            },
            1700,
          );

        }

      },
    );

    fullscreenButton.addEventListener(
      'click',
      async () => {

        try {

          if (
            document.fullscreenElement
          ) {

            await document.exitFullscreen();

            return;

          }

          await viewer.requestFullscreen();

        } catch {

          fullscreenButton.textContent =
            '✕ Indisponível';

          setTimeout(
            () => {

              fullscreenButton.textContent =
                '⛶ Tela cheia';

            },
            1700,
          );

        }

      },
    );

  </script>

</body>

</html>
`;
}

/* =========================================================
   RANKING PAGE
========================================================= */

app.get(
  '/ranking',
  (
    _req: Request,
    res: Response,
  ) => {

    res
      .type('html')
      .send(
        createRankingPage(),
      );

  },
);

function createRankingPage(): string {
  return `
<!doctype html>

<html lang="pt-BR">

<head>

  <meta charset="utf-8">

  <meta
    name="viewport"
    content="width=device-width, initial-scale=1"
  >

  <meta
    name="theme-color"
    content="#030806"
  >

  <meta
    name="color-scheme"
    content="dark"
  >

  <title>
    Ranking de Voz • Ghost Syndicate
  </title>

  <style>

    :root {

      color-scheme:
        dark;

      --bg:
        #030806;

      --panel:
        #07100a;

      --panel-2:
        #0a160e;

      --border:
        rgba(
          67,
          255,
          152,
          .14
        );

      --strong:
        rgba(
          67,
          255,
          152,
          .30
        );

      --green:
        #43ff98;

      --soft:
        #aaffc9;

      --text:
        #effff5;

      --muted:
        #769281;

      --muted-2:
        #526c5d;

    }

    * {
      box-sizing:
        border-box;
    }

    html,
    body {
      margin:
        0;

      min-height:
        100%;

      background:
        var(--bg);
    }

    body {

      min-height:
        100vh;

      overflow-x:
        hidden;

      color:
        var(--text);

      font-family:
        Inter,
        ui-sans-serif,
        system-ui,
        -apple-system,
        BlinkMacSystemFont,
        "Segoe UI",
        sans-serif;

      background:

        radial-gradient(
          circle at 10% 0%,
          rgba(
            67,
            255,
            152,
            .09
          ),
          transparent 30%
        ),

        radial-gradient(
          circle at 90% 15%,
          rgba(
            67,
            255,
            152,
            .04
          ),
          transparent 25%
        ),

        var(--bg);

    }

    body::before {

      content:
        "";

      position:
        fixed;

      inset:
        0;

      pointer-events:
        none;

      opacity:
        .012;

      background:
        repeating-linear-gradient(
          180deg,
          rgba(
            255,
            255,
            255,
            .25
          ) 0,
          rgba(
            255,
            255,
            255,
            .25
          ) 1px,
          transparent 1px,
          transparent 7px
        );

    }

    .page {

      position:
        relative;

      z-index:
        1;

      width:
        min(
          1180px,
          calc(
            100% - 28px
          )
        );

      margin:
        0 auto;

      padding:
        15px 0 30px;

    }

    .header {

      position:
        sticky;

      top:
        10px;

      z-index:
        30;

      display:
        flex;

      align-items:
        center;

      justify-content:
        space-between;

      gap:
        15px;

      padding:
        10px 13px;

      margin-bottom:
        13px;

      border:
        1px solid
        var(--border);

      border-radius:
        16px;

      background:
        rgba(
          5,
          13,
          8,
          .94
        );

      backdrop-filter:
        blur(
          16px
        );

    }

    .brand {

      display:
        flex;

      align-items:
        center;

      gap:
        10px;

    }

    .logo {

      width:
        42px;

      height:
        42px;

      display:
        grid;

      place-items:
        center;

      border:
        1px solid
        var(--strong);

      border-radius:
        13px;

      background:
        #0b1b11;

      font-size:
        21px;

    }

    .eyebrow {

      margin:
        0 0 2px;

      color:
        var(--green);

      font-size:
        8px;

      font-weight:
        950;

      letter-spacing:
        .20em;

      text-transform:
        uppercase;

    }

    .title {

      margin:
        0;

      font-size:
        14px;

      font-weight:
        900;

    }

    .header-right {

      display:
        flex;

      align-items:
        center;

      gap:
        7px;

      flex-wrap:
        wrap;

      justify-content:
        flex-end;

    }

    .pill {

      display:
        inline-flex;

      align-items:
        center;

      gap:
        7px;

      min-height:
        31px;

      padding:
        7px 10px;

      border:
        1px solid
        var(--border);

      border-radius:
        999px;

      background:
        #07120b;

      color:
        #84a18e;

      font-size:
        9px;

      font-weight:
        850;

    }

    .live-dot {

      width:
        7px;

      height:
        7px;

      border-radius:
        50%;

      background:
        var(--green);

      box-shadow:
        0 0 12px
        rgba(
          67,
          255,
          152,
          .85
        );

      animation:
        pulse
        1.7s
        ease-in-out
        infinite;

    }

    @keyframes pulse {

      0%,
      100% {

        opacity:
          .60;

        transform:
          scale(
            .9
          );

      }

      50% {

        opacity:
          1;

        transform:
          scale(
            1.08
          );

      }

    }

    .refresh {

      min-height:
        31px;

      display:
        inline-flex;

      align-items:
        center;

      padding:
        7px 10px;

      border:
        1px solid
        var(--border);

      border-radius:
        9px;

      background:
        #07120b;

      color:
        var(--soft);

      font:
        800 9px/1
        inherit;

    }

    .hero {

      display:
        grid;

      grid-template-columns:
        minmax(
          0,
          1fr
        )
        260px;

      gap:
        12px;

      margin-bottom:
        12px;

    }

    .hero-main,
    .leader-card {

      border:
        1px solid
        var(--border);

      border-radius:
        18px;

      background:
        linear-gradient(
          135deg,
          rgba(
            10,
            25,
            15,
            .97
          ),
          rgba(
            5,
            12,
            8,
            .98
          )
        );

    }

    .hero-main {

      padding:
        24px;

    }

    .hero-kicker {

      margin:
        0 0 7px;

      color:
        var(--green);

      font-size:
        9px;

      font-weight:
        950;

      letter-spacing:
        .18em;

      text-transform:
        uppercase;

    }

    .hero-main h2 {

      margin:
        0;

      font-size:
        clamp(
          32px,
          5vw,
          54px
        );

      line-height:
        .98;

      letter-spacing:
        -.06em;

    }

    .hero-main p {

      max-width:
        720px;

      margin:
        12px 0 0;

      color:
        var(--muted);

      font-size:
        12px;

      line-height:
        1.65;

    }

    .leader-card {

      padding:
        18px;

    }

    .small-label {

      margin:
        0 0 7px;

      color:
        var(--muted-2);

      font-size:
        8px;

      font-weight:
        950;

      letter-spacing:
        .15em;

      text-transform:
        uppercase;

    }

    .leader-time {

      color:
        var(--soft);

      font:
        900 27px/1
        ui-monospace,
        monospace;

    }

    .leader-name {

      margin-top:
        8px;

      font-size:
        13px;

      font-weight:
        900;

      overflow:
        hidden;

      text-overflow:
        ellipsis;

      white-space:
        nowrap;

    }

    .leader-meta {

      margin-top:
        16px;

      padding:
        10px;

      border:
        1px solid
        var(--border);

      border-radius:
        11px;

      background:
        rgba(
          67,
          255,
          152,
          .025
        );

      color:
        #83a08d;

      font-size:
        9px;

      line-height:
        1.55;

    }

    .podium {

      display:
        grid;

      grid-template-columns:
        repeat(
          3,
          minmax(
            0,
            1fr
          )
        );

      align-items:
        end;

      gap:
        12px;

      margin-bottom:
        12px;

    }

    .podium-card {

      min-height:
        188px;

      padding:
        17px;

      border:
        1px solid
        var(--border);

      border-radius:
        17px;

      background:
        linear-gradient(
          180deg,
          rgba(
            9,
            22,
            14,
            .96
          ),
          rgba(
            5,
            12,
            8,
            .98
          )
        );

      transition:
        transform
        .2s
        ease,
        border-color
        .2s
        ease;

    }

    .podium-card:hover {

      transform:
        translateY(
          -3px
        );

      border-color:
        var(--strong);

    }

    .first {

      min-height:
        222px;

      border-color:
        var(--strong);

      box-shadow:
        0 0 50px
        rgba(
          67,
          255,
          152,
          .04
        );

    }

    .rank-icon {

      font-size:
        24px;

    }

    .rank-label {

      margin-top:
        9px;

      color:
        var(--muted-2);

      font-size:
        8px;

      font-weight:
        950;

      letter-spacing:
        .14em;

      text-transform:
        uppercase;

    }

    .rank-name {

      margin-top:
        8px;

      font-size:
        14px;

      font-weight:
        900;

      white-space:
        nowrap;

      overflow:
        hidden;

      text-overflow:
        ellipsis;

    }

    .rank-time {

      margin-top:
        9px;

      color:
        var(--soft);

      font:
        900 18px/1.2
        ui-monospace,
        monospace;

    }

    .rank-live {

      display:
        inline-flex;

      align-items:
        center;

      gap:
        5px;

      margin-top:
        11px;

      padding:
        5px 7px;

      border:
        1px solid
        var(--border);

      border-radius:
        999px;

      background:
        rgba(
          67,
          255,
          152,
          .025
        );

      color:
        #80a98f;

      font-size:
        8px;

      font-weight:
        900;

    }

    .panel {

      overflow:
        hidden;

      border:
        1px solid
        var(--border);

      border-radius:
        18px;

      background:
        var(--panel);

      box-shadow:
        0 25px 90px
        rgba(
          0,
          0,
          0,
          .25
        );

    }

    .panel-head {

      display:
        flex;

      align-items:
        center;

      justify-content:
        space-between;

      gap:
        12px;

      padding:
        13px 15px;

      border-bottom:
        1px solid
        var(--border);

      background:
        var(--panel-2);

    }

    .panel-title {

      margin:
        0;

      display:
        flex;

      align-items:
        center;

      gap:
        8px;

      font-size:
        11px;

      font-weight:
        900;

    }

    .panel-title span {

      color:
        var(--green);

    }

    .updated {

      color:
        var(--muted-2);

      font-size:
        8px;

    }

    .list {

      display:
        grid;

      gap:
        1px;

    }

    .row {

      display:
        grid;

      grid-template-columns:
        52px
        minmax(
          0,
          1fr
        )
        minmax(
          180px,
          .8fr
        )
        auto;

      align-items:
        center;

      gap:
        13px;

      min-height:
        75px;

      padding:
        11px 14px;

      background:
        #09100c;

      transition:
        background
        .18s
        ease;

    }

    .row:hover {

      background:
        #0c1610;

    }

    .row.active {

      background:
        linear-gradient(
          90deg,
          rgba(
            67,
            255,
            152,
            .035
          ),
          #09100c
        );

    }

    .position {

      width:
        37px;

      height:
        37px;

      display:
        grid;

      place-items:
        center;

      border:
        1px solid
        var(--border);

      border-radius:
        11px;

      background:
        #0b1710;

      color:
        #a1b9aa;

      font-size:
        11px;

      font-weight:
        950;

    }

    .top-position {

      color:
        var(--green);

      border-color:
        var(--strong);

    }

    .member {

      min-width:
        0;

    }

    .member-name {

      overflow:
        hidden;

      text-overflow:
        ellipsis;

      white-space:
        nowrap;

      font-size:
        12px;

      font-weight:
        850;

    }

    .member-id {

      margin-top:
        3px;

      overflow:
        hidden;

      text-overflow:
        ellipsis;

      white-space:
        nowrap;

      color:
        var(--muted-2);

      font:
        8px
        ui-monospace,
        monospace;

    }

    .bar-area {

      min-width:
        0;

    }

    .bar-top {

      display:
        flex;

      justify-content:
        space-between;

      gap:
        8px;

      margin-bottom:
        6px;

      color:
        #66806f;

      font-size:
        8px;

    }

    .bar {

      width:
        100%;

      height:
        6px;

      overflow:
        hidden;

      border-radius:
        999px;

      background:
        #101a14;

    }

    .bar-fill {

      height:
        100%;

      border-radius:
        inherit;

      background:
        linear-gradient(
          90deg,
          #1f9f60,
          #43ff98
        );

      box-shadow:
        0 0 15px
        rgba(
          67,
          255,
          152,
          .23
        );

      transition:
        width
        .4s
        ease;

    }

    .time {

      min-width:
        115px;

      text-align:
        right;

      color:
        var(--soft);

      font:
        900 12px/1.2
        ui-monospace,
        monospace;

    }

    .live {

      display:
        flex;

      align-items:
        center;

      justify-content:
        flex-end;

      gap:
        5px;

      margin-top:
        5px;

      color:
        #78a58a;

      font:
        800 8px
        inherit;

    }

    .empty {

      padding:
        35px;

      text-align:
        center;

      color:
        var(--muted);

      font-size:
        11px;

      line-height:
        1.6;

    }

    .error {

      color:
        #d48a8a;

    }

    .footer {

      display:
        flex;

      justify-content:
        space-between;

      gap:
        10px;

      margin-top:
        9px;

      color:
        #4f6859;

      font-size:
        8px;

      flex-wrap:
        wrap;

    }

    .footer strong {
      color:
        #78a58a;
    }

    @media (
      max-width: 900px
    ) {

      .hero {

        grid-template-columns:
          1fr;

      }

      .podium {

        grid-template-columns:
          1fr;

      }

      .podium-card,
      .first {

        min-height:
          170px;

      }

      .row {

        grid-template-columns:
          45px
          minmax(
            0,
            1fr
          )
          auto;

      }

      .bar-area {

        display:
          none;

      }

    }

    @media (
      max-width: 620px
    ) {

      .page {

        width:
          calc(
            100% - 12px
          );

        padding-top:
          6px;

      }

      .header {

        position:
          relative;

        top:
          auto;

        align-items:
          flex-start;

        flex-direction:
          column;

      }

      .header-right {

        justify-content:
          flex-start;

      }

      .panel-head {

        align-items:
          flex-start;

        flex-direction:
          column;

      }

      .row {

        grid-template-columns:
          35px
          minmax(
            0,
            1fr
          )
          auto;

        gap:
          8px;

        padding:
          10px;

      }

      .position {

        width:
          31px;

        height:
          31px;

      }

      .time {

        min-width:
          auto;

        font-size:
          10px;

      }

      .member-name {

        font-size:
          11px;

      }

      .footer {

        flex-direction:
          column;

      }

    }

  </style>

</head>

<body>

  <main class="page">

    <header class="header">

      <div class="brand">

        <div class="logo">
          👻
        </div>

        <div>

          <p class="eyebrow">
            GHOST SYNDICATE
          </p>

          <p class="title">
            Ranking de Voz
          </p>

        </div>

      </div>

      <div class="header-right">

        <div class="pill">

          <span class="live-dot"></span>

          Atualização em tempo real

        </div>

        <div
          id="refresh"
          class="refresh"
        >
          Monitorando
        </div>

      </div>

    </header>

    <section class="hero">

      <div class="hero-main">

        <p class="hero-kicker">
          Atividade da equipe
        </p>

        <h2>
          Ranking de horas em call.
        </h2>

        <p>
          O tempo é contabilizado automaticamente
          pelas sessões de voz. Membros atualmente
          em call continuam acumulando tempo em
          tempo real.
        </p>

      </div>

      <aside class="leader-card">

        <p class="small-label">
          Líder atual
        </p>

        <div
          id="leaderTime"
          class="leader-time"
        >
          00:00:00
        </div>

        <div
          id="leaderName"
          class="leader-name"
        >
          Carregando...
        </div>

        <div
          id="leaderMeta"
          class="leader-meta"
        >
          Carregando ranking...
        </div>

      </aside>

    </section>

    <section
      id="podium"
      class="podium"
    >

      <article class="podium-card">
        <div class="empty">
          Carregando...
        </div>
      </article>

      <article class="podium-card first">
        <div class="empty">
          Carregando...
        </div>
      </article>

      <article class="podium-card">
        <div class="empty">
          Carregando...
        </div>
      </article>

    </section>

    <section class="panel">

      <header class="panel-head">

        <h3 class="panel-title">

          <span>
            🎙️
          </span>

          Ranking completo

        </h3>

        <span
          id="updated"
          class="updated"
        >
          Aguardando atualização...
        </span>

      </header>

      <div
        id="list"
        class="list"
      >

        <div class="empty">
          Carregando ranking...
        </div>

      </div>

    </section>

    <footer class="footer">

      <span>

        Ghost Syndicate
        •
        <strong>Organização</strong>
        •
        <strong>Lealdade</strong>
        •
        <strong>Resultado</strong>

      </span>

      <span>
        Voice Tracker
      </span>

    </footer>

  </main>

  <script>

    let rankingData = [];

    function escapeHtml(
      value,
    ) {

      return String(
        value ?? '',
      )
        .replaceAll(
          '&',
          '&amp;',
        )
        .replaceAll(
          '<',
          '&lt;',
        )
        .replaceAll(
          '>',
          '&gt;',
        )
        .replaceAll(
          '"',
          '&quot;',
        )
        .replaceAll(
          "'",
          '&#039;',
        );

    }

    function formatDuration(
      seconds,
    ) {

      const safe =
        Math.max(
          0,
          Math.floor(
            Number(
              seconds || 0,
            ),
          ),
        );

      const hours =
        Math.floor(
          safe /
            3600,
        );

      const minutes =
        Math.floor(
          (
            safe %
            3600
          ) /
            60,
        );

      const secs =
        safe %
        60;

      return (
        hours +
        'h ' +
        minutes +
        'min ' +
        secs +
        's'
      );

    }

    function formatClock(
      seconds,
    ) {

      const safe =
        Math.max(
          0,
          Math.floor(
            Number(
              seconds || 0,
            ),
          ),
        );

      const hours =
        Math.floor(
          safe /
            3600,
        )
          .toString()
          .padStart(
            2,
            '0',
          );

      const minutes =
        Math.floor(
          (
            safe %
            3600
          ) /
            60,
        )
          .toString()
          .padStart(
            2,
            '0',
          );

      const secs =
        (
          safe %
          60
        )
          .toString()
          .padStart(
            2,
            '0',
          );

      return (
        hours +
        ':' +
        minutes +
        ':' +
        secs
      );

    }

    function renderSummary(
      ranking,
    ) {

      const leader =
        ranking[0];

      const leaderTime =
        document.getElementById(
          'leaderTime',
        );

      const leaderName =
        document.getElementById(
          'leaderName',
        );

      const leaderMeta =
        document.getElementById(
          'leaderMeta',
        );

      if (
        !leader
      ) {

        leaderTime.textContent =
          '00:00:00';

        leaderName.textContent =
          'Ainda não há registros';

        leaderMeta.textContent =
          'Nenhum membro possui tempo registrado.';

        return;

      }

      leaderTime.textContent =
        formatClock(
          leader.seconds,
        );

      leaderName.textContent =
        leader.name;

      const active =
        ranking.filter(
          (
            item,
          ) =>
            item.active,
        ).length;

      leaderMeta.textContent =
        ranking.length +
        ' membros registrados • ' +
        active +
        ' em call agora';

    }

    function renderPodium(
      ranking,
    ) {

      const container =
        document.getElementById(
          'podium',
        );

      const positions = [
        {
          index:
            1,

          emoji:
            '🥈',
        },

        {
          index:
            0,

          emoji:
            '🥇',

          first:
            true,
        },

        {
          index:
            2,

          emoji:
            '🥉',
        },
      ];

      container.innerHTML =
        positions
          .map(
            (
              position,
            ) => {

              const member =
                ranking[
                  position.index
                ];

              if (
                !member
              ) {

                return (
                  '<article class="podium-card ' +
                  (
                    position.first
                      ? 'first'
                      : ''
                  ) +
                  '">' +

                  '<div class="empty">' +
                  'Sem registro' +
                  '</div>' +

                  '</article>'
                );

              }

              return (
                '<article class="podium-card ' +
                (
                  position.first
                    ? 'first'
                    : ''
                ) +
                '">' +

                '<div class="rank-icon">' +
                position.emoji +
                '</div>' +

                '<div class="rank-label">' +
                'POSIÇÃO ' +
                member.position +
                '</div>' +

                '<div class="rank-name">' +
                escapeHtml(
                  member.name,
                ) +
                '</div>' +

                '<div class="rank-time">' +
                formatDuration(
                  member.seconds,
                ) +
                '</div>' +

                (
                  member.active
                    ? (
                      '<div class="rank-live">' +
                      '<span class="live-dot"></span>' +
                      ' AO VIVO' +
                      '</div>'
                    )
                    : ''
                ) +

                '</article>'
              );

            },
          )
          .join('');

    }

    function renderList(
      ranking,
    ) {

      const list =
        document.getElementById(
          'list',
        );

      if (
        ranking.length ===
        0
      ) {

        list.innerHTML =
          '<div class="empty">' +
          'Ainda não existem registros de voz.' +
          '</div>';

        return;

      }

      const leaderSeconds =
        Math.max(
          1,
          Number(
            ranking[0]?.seconds ||
              0,
          ),
        );

      list.innerHTML =
        ranking
          .map(
            (
              member,
              index,
            ) => {

              const percent =
                Math.min(
                  100,
                  (
                    member.seconds /
                    leaderSeconds
                  ) *
                    100,
                );

              return (
                '<article class="row ' +
                (
                  member.active
                    ? 'active'
                    : ''
                ) +
                '">' +

                '<div class="position ' +
                (
                  index < 3
                    ? 'top-position'
                    : ''
                ) +
                '">' +
                (
                  index + 1
                ) +
                '</div>' +

                '<div class="member">' +

                '<div class="member-name">' +
                escapeHtml(
                  member.name,
                ) +
                '</div>' +

                '<div class="member-id">' +
                member.memberId +
                '</div>' +

                '</div>' +

                '<div class="bar-area">' +

                '<div class="bar-top">' +

                '<span>' +
                'TEMPO' +
                '</span>' +

                '<span>' +
                percent.toFixed(
                  0,
                ) +
                '%' +
                '</span>' +

                '</div>' +

                '<div class="bar">' +

                '<div ' +
                'class="bar-fill" ' +
                'style="width:' +
                percent.toFixed(
                  2,
                ) +
                '%">' +
                '</div>' +

                '</div>' +

                '</div>' +

                '<div class="time">' +

                formatDuration(
                  member.seconds,
                ) +

                (
                  member.active
                    ? (
                      '<div class="live">' +
                      '<span class="live-dot"></span>' +
                      ' AO VIVO' +
                      '</div>'
                    )
                    : ''
                ) +

                '</div>' +

                '</article>'
              );

            },
          )
          .join('');

    }

    async function loadRanking() {

      try {

        const response =
          await fetch(
            '/api/ranking/live',
            {
              cache:
                'no-store',
            },
          );

        if (
          !response.ok
        ) {

          throw new Error(
            'Falha na API.',
          );

        }

        const data =
          await response.json();

        rankingData =
          Array.isArray(
            data.ranking,
          )
            ? data.ranking
            : [];

        renderSummary(
          rankingData,
        );

        renderPodium(
          rankingData,
        );

        renderList(
          rankingData,
        );

        document.getElementById(
          'updated',
        ).textContent =
          'Atualizado agora';

        const refresh =
          document.getElementById(
            'refresh',
          );

        refresh.textContent =
          '✓ Monitorando';

      } catch (
        error
      ) {

        console.error(
          '[RANKING PAGE]',
          error,
        );

        document.getElementById(
          'list',
        ).innerHTML =
          '<div class="empty error">' +
          'Não foi possível atualizar o ranking.' +
          '</div>';

        document.getElementById(
          'refresh',
        ).textContent =
          '⚠ Erro';

      }

    }

    loadRanking();

    /*
     * Atualiza a cada segundo para que
     * o tempo AO VIVO acompanhe a call.
     */

    setInterval(
      loadRanking,
      1000,
    );

  </script>

</body>

</html>
`;
}

/* =========================================================
   DASHBOARD
========================================================= */

app.get(
  '/',
  (
    _req: Request,
    res: Response,
  ) => {

    res
      .type('html')
      .send(
        createDashboardPage(),
      );

  },
);

function createDashboardPage(): string {
  return `
<!doctype html>

<html lang="pt-BR">

<head>

  <meta charset="utf-8">

  <meta
    name="viewport"
    content="width=device-width, initial-scale=1"
  >

  <meta
    name="theme-color"
    content="#050a07"
  >

  <meta
    name="color-scheme"
    content="dark"
  >

  <title>
    Ghost Syndicate
  </title>

  <style>

    :root {

      color-scheme:
        dark;

      --bg:
        #050a07;

      --panel:
        #0b120e;

      --border:
        #173024;

      --green:
        #43ff98;

      --text:
        #f0fff6;

      --muted:
        #7c9787;

    }

    * {
      box-sizing:
        border-box;
    }

    body {

      margin:
        0;

      min-height:
        100vh;

      color:
        var(--text);

      background:
        radial-gradient(
          circle at 10% 0%,
          rgba(
            67,
            255,
            152,
            .08
          ),
          transparent 30%
        ),
        var(--bg);

      font-family:
        Inter,
        system-ui,
        sans-serif;

    }

    main {

      width:
        min(
          1200px,
          calc(
            100% - 32px
          )
        );

      margin:
        0 auto;

      padding:
        30px 0 50px;

    }

    .topbar {

      display:
        flex;

      align-items:
        center;

      justify-content:
        space-between;

      gap:
        18px;

      margin-bottom:
        35px;

    }

    .brand {

      display:
        flex;

      align-items:
        center;

      gap:
        12px;

    }

    .logo {

      width:
        50px;

      height:
        50px;

      display:
        grid;

      place-items:
        center;

      border:
        1px solid
        rgba(
          67,
          255,
          152,
          .22
        );

      border-radius:
        16px;

      background:
        #0d2417;

      font-size:
        24px;

    }

    .eyebrow {

      margin:
        0 0 3px;

      color:
        var(--green);

      font-size:
        9px;

      font-weight:
        900;

      letter-spacing:
        .18em;

      text-transform:
        uppercase;

    }

    .brand h1 {

      margin:
        0;

      font-size:
        20px;

    }

    .online {

      display:
        inline-flex;

      align-items:
        center;

      gap:
        7px;

      padding:
        9px 13px;

      border:
        1px solid
        var(--border);

      border-radius:
        999px;

      color:
        #8aa595;

      font-size:
        10px;

      background:
        rgba(
          7,
          18,
          11,
          .85
        );

    }

    .online-dot {

      width:
        7px;

      height:
        7px;

      border-radius:
        50%;

      background:
        var(--green);

      box-shadow:
        0 0 12px
        rgba(
          67,
          255,
          152,
          .85
        );

    }

    .hero {

      margin-bottom:
        22px;

    }

    .hero h2 {

      margin:
        0;

      max-width:
        800px;

      font-size:
        clamp(
          34px,
          5vw,
          58px
        );

      line-height:
        .98;

      letter-spacing:
        -.06em;

    }

    .hero p {

      max-width:
        720px;

      margin:
        14px 0 0;

      color:
        var(--muted);

      font-size:
        14px;

      line-height:
        1.65;

    }

    .cards {

      display:
        grid;

      grid-template-columns:
        repeat(
          3,
          minmax(
            0,
            1fr
          )
        );

      gap:
        13px;

    }

    .card {

      padding:
        20px;

      border:
        1px solid
        var(--border);

      border-radius:
        19px;

      background:
        linear-gradient(
          180deg,
          rgba(
            12,
            25,
            17,
            .94
          ),
          rgba(
            6,
            13,
            9,
            .98
          )
        );

    }

    .card-top {

      display:
        flex;

      align-items:
        center;

      justify-content:
        space-between;

      margin-bottom:
        18px;

    }

    .label {

      margin:
        0;

      color:
        #708d7c;

      font-size:
        9px;

      font-weight:
        900;

      letter-spacing:
        .13em;

      text-transform:
        uppercase;

    }

    .value {

      font-size:
        28px;

      font-weight:
        900;

    }

    .ranking {

      margin-top:
        16px;

    }

    .section-title {

      margin:
        0 0 15px;

      font-size:
        18px;

    }

    .rank-list {

      display:
        grid;

      gap:
        8px;

    }

    .rank-row {

      display:
        grid;

      grid-template-columns:
        38px
        1fr
        auto;

      align-items:
        center;

      gap:
        10px;

      min-height:
        55px;

      padding:
        9px 11px;

      border:
        1px solid
        #14271e;

      border-radius:
        14px;

      background:
        rgba(
          255,
          255,
          255,
          .012
        );

    }

    .rank-pos {

      width:
        30px;

      height:
        30px;

      display:
        grid;

      place-items:
        center;

      border-radius:
        9px;

      background:
        #0d1a13;

      color:
        #a6c4b1;

      font-size:
        11px;

      font-weight:
        900;

    }

    .rank-name {

      font-size:
        12px;

      font-weight:
        800;

    }

    .rank-sub {

      margin-top:
        2px;

      color:
        #5e7968;

      font-size:
        10px;

    }

    .rank-time {

      color:
        #a6bbae;

      font-size:
        11px;

      font-weight:
        750;

      white-space:
        nowrap;

    }

    .empty {

      color:
        #637f6c;

      font-size:
        11px;

    }

    footer {

      margin-top:
        30px;

      text-align:
        center;

      color:
        #4e6858;

      font-size:
        9px;

    }

    @media (
      max-width: 800px
    ) {

      main {

        width:
          calc(
            100% - 20px
          );

      }

      .topbar {

        align-items:
          flex-start;

        flex-direction:
          column;

      }

      .cards {

        grid-template-columns:
          1fr;

      }

    }

  </style>

</head>

<body>

  <main>

    <header class="topbar">

      <div class="brand">

        <div class="logo">
          👻
        </div>

        <div>

          <p class="eyebrow">
            Ghost Syndicate
          </p>

          <h1>
            Painel Operacional
          </h1>

        </div>

      </div>

      <div class="online">

        <span
          class="online-dot"
        ></span>

        Sistema online

      </div>

    </header>

    <section class="hero">

      <h2>
        Controle da operação.
      </h2>

      <p>
        Caixa, metas, atividade
        e horas em call em um único
        painel da Ghost Syndicate.
      </p>

    </section>

    <section class="cards">

      <article class="card">

        <div class="card-top">

          <p class="label">
            Saldo atual
          </p>

          <span>
            🏦
          </span>

        </div>

        <div
          id="cash"
          class="value"
        >
          —
        </div>

      </article>

      <article class="card">

        <div class="card-top">

          <p class="label">
            Meta diária
          </p>

          <span>
            🎯
          </span>

        </div>

        <div
          id="goal"
          class="value"
        >
          —
        </div>

      </article>

      <article class="card">

        <div class="card-top">

          <p class="label">
            Reserva
          </p>

          <span>
            🛡️
          </span>

        </div>

        <div
          id="reserve"
          class="value"
        >
          —
        </div>

      </article>

    </section>

    <section class="card ranking">

      <h3 class="section-title">
        🎙️ Ranking de horas em call
      </h3>

      <div
        id="ranking"
        class="rank-list"
      >

        <div class="empty">
          Carregando...
        </div>

      </div>

    </section>

    <footer>

      Ghost Syndicate
      • Organização
      • Lealdade
      • Resultado

    </footer>

  </main>

  <script>

    function money(
      value,
    ) {

      return Number(
        value || 0,
      ).toLocaleString(
        'pt-BR',
        {
          style:
            'currency',

          currency:
            'BRL',

          maximumFractionDigits:
            0,
        },
      );

    }

    function duration(
      seconds,
    ) {

      const safe =
        Math.floor(
          Number(
            seconds || 0,
          ),
        );

      const hours =
        Math.floor(
          safe /
            3600,
        );

      const minutes =
        Math.floor(
          (
            safe %
            3600
          ) /
            60,
        );

      return (
        hours +
        'h ' +
        minutes +
        'min'
      );

    }

    function escapeHtml(
      value,
    ) {

      return String(
        value ?? '',
      )
        .replaceAll(
          '&',
          '&amp;',
        )
        .replaceAll(
          '<',
          '&lt;',
        )
        .replaceAll(
          '>',
          '&gt;',
        )
        .replaceAll(
          '"',
          '&quot;',
        )
        .replaceAll(
          "'",
          '&#039;',
        );

    }

    async function load() {

      try {

        const response =
          await fetch(
            '/api/overview',
            {
              cache:
                'no-store',
            },
          );

        if (
          !response.ok
        ) {

          throw new Error(
            'Falha na API.',
          );

        }

        const data =
          await response.json();

        const guild =
          data.guild;

        document.getElementById(
          'cash',
        ).textContent =
          money(
            guild?.cashBalance,
          );

        document.getElementById(
          'goal',
        ).textContent =
          guild?.dailyGoal > 0
            ? money(
                guild.dailyGoal,
              )
            : 'Não definida';

        document.getElementById(
          'reserve',
        ).textContent =
          guild?.reserve > 0
            ? money(
                guild.reserve,
              )
            : 'Não definida';

        const ranking =
          Array.isArray(
            data.topVoice,
          )
            ? data.topVoice
            : [];

        const element =
          document.getElementById(
            'ranking',
          );

        if (
          ranking.length ===
          0
        ) {

          element.innerHTML =
            '<div class="empty">' +
            'Nenhum registro de voz ainda.' +
            '</div>';

          return;

        }

        element.innerHTML =
          ranking
            .map(
              (
                member,
                index,
              ) => {

                return (
                  '<div class="rank-row">' +

                  '<div class="rank-pos">' +
                    (
                      index + 1
                    ) +
                  '</div>' +

                  '<div>' +

                  '<div class="rank-name">' +
                    escapeHtml(
                      member.name,
                    ) +
                  '</div>' +

                  '<div class="rank-sub">' +
                    (
                      member.active
                        ? '🟢 AO VIVO'
                        : 'Participação em voz'
                    ) +
                  '</div>' +

                  '</div>' +

                  '<div class="rank-time">' +
                    duration(
                      member.seconds,
                    ) +
                  '</div>' +

                  '</div>'
                );

              },
            )
            .join('');

      } catch (
        error
      ) {

        console.error(
          '[DASHBOARD]',
          error,
        );

        document.getElementById(
          'ranking',
        ).innerHTML =
          '<div class="empty">' +
          'Não foi possível carregar os dados.' +
          '</div>';

      }

    }

    load();

    setInterval(
      load,
      5000,
    );

  </script>

</body>

</html>
`;
}

/* =========================================================
   ERROR HANDLER
========================================================= */

app.use(
  (
    error: unknown,
    _req: Request,
    res: Response,
    _next: NextFunction,
  ) => {

    console.error(
      '[WEB ERROR]',
      error,
    );

    if (
      res.headersSent
    ) {
      return;
    }

    res
      .status(500)
      .json({
        error:
          'Erro interno do servidor.',
      });

  },
);

/* =========================================================
   NOT FOUND
========================================================= */

app.use(
  (
    _req: Request,
    res: Response,
  ) => {

    res
      .status(404)
      .json({
        error:
          'Rota não encontrada.',
      });

  },
);

/* =========================================================
   START
========================================================= */

const server =
  app.listen(
    PORT,
    () => {

      console.log('');

      console.log(
        '👻 Ghost Syndicate Web',
      );

      console.log(
        `🌐 http://localhost:${PORT}`,
      );

      console.log(
        '📜 Transcripts:',
      );

      console.log(
        `   http://localhost:${PORT}/transcripts/ID`,
      );

      console.log(
        '🎙️ Ranking:',
      );

      console.log(
        `   http://localhost:${PORT}/ranking`,
      );

      console.log('');

    },
  );

/* =========================================================
   SHUTDOWN
========================================================= */

async function shutdown(
  signal: string,
): Promise<void> {

  console.log('');

  console.log(
    `🛑 Recebido ${signal}. Encerrando servidor...`,
  );

  server.close(
    async () => {

      try {

        await db.$disconnect();

        console.log(
          '✅ Banco desconectado.',
        );

        console.log(
          '✅ Servidor encerrado.',
        );

        process.exit(
          0,
        );

      } catch (
        error
      ) {

        console.error(
          '❌ Erro ao encerrar:',
          error,
        );

        process.exit(
          1,
        );

      }

    },
  );
}

process.on(
  'SIGINT',
  () => {

    void shutdown(
      'SIGINT',
    );

  },
);

process.on(
  'SIGTERM',
  () => {

    void shutdown(
      'SIGTERM',
    );

  },
);